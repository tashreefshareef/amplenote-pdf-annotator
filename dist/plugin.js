...
${d}`}function wn(t,l){let a=(t||"").split(`
`),s=ke(a,G);if(!s)return null;let i=vn(a.slice(s.start+1,s.end).join(`
...
<div id="pdfa-root"${p?' class="pdfa-collapsed-mode"':""}>
  <div class="pdfa-collapsed">
    ...
    <span class="pdfa-collapsed-name">${bn(l)}</span>
...
<script>window.__PDFA_CONFIG = ${yn(S)};
window.__PDFA_GEOM = (${Pe.toString()})();
window.__PDFA_ANNOTATIONS = (${De.toString()})();
window.__PDFA_EXPORT = (${Ue.toString()})();<\/script>
<script>(${Ct.toString()})();<\/script>`}var En={noteOption:{"Annotate PDF":async function(t,l){return at(t,l,t.context.pluginUUID)},"Debug: dump note markdown":async function(t,l){return it(t,l)}},insertText:async function(t){return lt(t,t.context.noteUUID,t.context.pluginUUID)},linkTarget:async function(t,...l){return st(t,l[0])},renderEmbed:function(t,...l){let{attachmentUUID:a,page:s,highlightId:i,collapsed:d,attachmentName:o}=re(l[0]);return a?Nt({attachmentUUID:a,page:s,highlightId:i,collapsed:d,attachmentName:o,lightDarkMode:t.context.lightDarkMode,pluginUUID:t.context.pluginUUID,noteUUID:t.context.noteUUID}):`<p style="font:13px sans-serif;padding:12px">
        This viewer isn't linked to a PDF. Re-run <b>Annotate PDF</b> from the note menu.
      </p>`},onEmbedCall:async function(t,...l){return yt(t,l[0])}},Nn=En;return ln(Sn);})();
