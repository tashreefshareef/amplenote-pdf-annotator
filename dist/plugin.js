...
`),d=`plugin://${l}`,o=i.findIndex(T=>T.includes(d)&&T.includes(`att=${r}`));if(o===-1)return null;let p=i[o],A=p.match(/data="(plugin:\/\/[^"]*)"/);if(!A)return null;let g=A[1],S=g.indexOf("?"),N=S===-1?"":g.slice(S+1),x={...ae(N),attachmentUUID:r,...s},u=nt(x),f=u?`plugin://${l}?${u}`:`plugin://${l}`,b=i.slice(),y=p.replace(A[0],`data="${f}"`),E=tt(x.collapsed),C=y.match(/data-aspect-ratio="[^"]*"/);return y=C?y.replace(C[0],`data-aspect-ratio="${E}"`):y.replace(/\s*\/>\s*$/,` data-aspect-ratio="${E}" />`),b[o]=y,b.join(`
