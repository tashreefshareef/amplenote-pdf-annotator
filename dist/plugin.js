...
`),d=`plugin://${s}`,o=r.findIndex(k=>k.includes(d)&&k.includes(`att=${a}`));if(o===-1)return null;let h=r[o],b=h.match(/data="(plugin:\/\/[^"]*)"/);if(!b)return null;let v=b[1],I=v.indexOf("?"),S=I===-1?"":v.slice(I+1),E={...ae(S),attachmentUUID:a,...l},u=tt(E),f=u?`plugin://${s}?${u}`:`plugin://${s}`,w=r.slice(),x=h.replace(b[0],`data="${f}"`),y=et(E.collapsed),C=x.match(/data-aspect-ratio="[^"]*"/);return x=C?x.replace(C[0],`data-aspect-ratio="${y}"`):x.replace(/\s*\/>\s*$/,` data-aspect-ratio="${y}" />`),w[o]=x,w.join(`
...
`),l=Te(a,W);if(!l)return null;let r=a.slice(0,l.start).join(`
...
`),l=Te(a,W);if(!l)return null;let r=pn(a.slice(l.start+1,l.end).join(`
...
`+o:""}`}function q(t,s){return s.noteUUID||t.context.noteUUID}async function vt(t,s,a){try{let l=await t.getNoteAttachments({uuid:s}),r=Array.isArray(l)&&l.find(d=>d&&d.uuid===a);return r?r.name:""}catch{return""}}async function me(t,s,a,l){let r=await ge(t,s,a),d=l(r);return d!==r&&await ft(t,s,a,d),{highlights:d}}function wt(t){if(t&&typeof t=="object")return t;if(typeof t!="string")return{};let s=t.trim();if(!s.startsWith("{"))return{action:s};try{return JSON.parse(s)}catch{return{action:s}}}async function xt(t,s){return JSON.stringify(await fn(t,wt(s)))}async function fn(t,s){let a=wt(s);switch(a.action){case"getPdfUrl":{let l=a.attachmentUUID;if(!l)return{error:"No attachment specified for this viewer."};try{return{url:await Ze(t,l),name:await vt(t,q(t,a),l)}}catch(r){return{error:`Could not load the PDF: ${r.message}`}}}case"loadHighlights":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return{highlights:await ge(t,q(t,a),a.attachmentUUID)}}catch(l){return{error:`Could not load highlights: ${l.message}`}}}case"addHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{let l=fe(a.highlight||{});return await me(t,q(t,a),a.attachmentUUID,r=>r.concat([l]))}catch(l){return{error:`Could not save the highlight: ${l.message}`}}}case"recolorHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await me(t,q(t,a),a.attachmentUUID,l=>Ne(l,a.id,r=>dt(r,a.color)))}catch(l){return{error:`Could not change the highlight color: ${l.message}`}}}case"setHighlightNote":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await me(t,q(t,a),a.attachmentUUID,l=>Ne(l,a.id,r=>ct(r,a.note)))}catch(l){return{error:`Could not save the note: ${l.message}`}}}case"removeHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await me(t,q(t,a),a.attachmentUUID,l=>ht(l,a.id))}catch(l){return{error:`Could not remove the highlight: ${l.message}`}}}case"sendToNote":{if(!a.content)return{error:"Nothing to send."};try{let l={uuid:q(t,a)},r=await t.getNoteContent(l),d=mt(r,a.content);return d===null?await t.insertNoteContent(l,`
`+a.content+`
`,{atEnd:!0}):await t.replaceNoteContent(l,d),{ok:!0}}catch(l){return{error:`Could not add this to the note: ${l.message}`}}}case"removeViewer":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!a.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=q(t,a),r=await t.getNoteContent({uuid:l}),d=pe(r,a.pluginUUID,a.attachmentUUID);return d===null?{error:"Could not find this viewer's block in the note - it may already be removed."}:(await t.replaceNoteContent({uuid:l},d),await gt(t,l,a.attachmentUUID),{ok:!0})}catch(l){return{error:`Could not remove this viewer: ${l.message}`}}}case"getViewerSummary":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};let l=q(t,a),r=await vt(t,l,a.attachmentUUID);try{let d=await ge(t,l,a.attachmentUUID);return{name:r,count:d.length}}catch{return{name:r,count:0}}}case"setCollapsed":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!a.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=q(t,a),r=await t.getNoteContent({uuid:l}),d=ot(r,a.pluginUUID,a.attachmentUUID,a.collapsed);return d===null?{ok:!1}:(await t.replaceNoteContent({uuid:l},d),{ok:!0})}catch(l){return{error:`Could not resize this viewer: ${l.message}`}}}case"exportAll":{if(!a.noteName)return{error:"Missing destination note name."};try{let l=await t.findNote({name:a.noteName}),r=l?l.uuid:await t.createNote(a.noteName);return await t.replaceNoteContent({uuid:r},a.content||""),{ok:!0,noteUUID:r}}catch(l){return{error:`Could not export highlights: ${l.message}`}}}case"ping":return{ok:!0};default:return{error:`Unknown embed action: ${String(a.action)}`}}}function Ie(){function t(u,f){return{x:u.left-f.left,y:u.top-f.top,width:u.width,height:u.height}}function s(u,f){return{x:Math.min(u[0],f[0]),y:Math.min(u[1],f[1]),width:Math.abs(f[0]-u[0]),height:Math.abs(f[1]-u[1])}}function a(u,f){var w=Math.pow(10,f===void 0?2:f),x=function(y){return Math.round(y*w)/w};return{x:x(u.x),y:x(u.y),width:x(u.width),height:x(u.height)}}function l(u){return u.width>.01&&u.height>.01}function r(u,f,w){for(var x=String(u??""),y=Math.max(0,f===void 0?0:f),C=Math.min(x.length,w===void 0?x.length:w),k=function($){return $===""||/\s/.test($)},R=[],U=y;U<C;){for(;U<C&&k(x.charAt(U));)U++;if(U>=C)break;for(var j=U;U<C&&!k(x.charAt(U));)U++;R.push({start:j,end:U})}return R}function d(u){for(var f=1/0,w=1/0,x=-1/0,y=-1/0,C=0;C<(u?u.length:0);C++){var k=u[C];l(k)&&(f=Math.min(f,k.left),w=Math.min(w,k.top),x=Math.max(x,k.left+k.width),y=Math.max(y,k.top+k.height))}return isFinite(f)?{left:f,top:w,width:x-f,height:y-w}:null}function o(u,f,w){for(var x=[],y=0;y<u.length;y++){var C=t(u[y],f);if(l(C)){var k=w(C.x,C.y),R=w(C.x+C.width,C.y+C.height),U=a(s(k,R));l(U)&&x.push(U)}}return x}function h(u,f){var w=f(u.x,u.y),x=f(u.x+u.width,u.y+u.height);return s(w,x)}function b(u,f,w){var x=f.right-f.left,y=f.bottom-f.top;if(x<=0||y<=0)return null;var C=u.x2-u.x1,k=u.y2-u.y1,R=u.x1+(w.left-f.left)/x*C,U=u.x2-(f.right-w.right)/x*C,j=u.y1+(w.bottom-f.bottom)/y*k,$=u.y2-(f.top-w.top)/y*k;return{x:R,y:j,width:U-R,height:$-j}}function v(u,f){var w=Math.min(u.y+u.height,f.y+f.height)-Math.max(u.y,f.y);return w>.5*Math.min(u.height,f.height)}function I(u,f){var w=f===void 0?.6:f;if(!u||u.length<2)return(u||[]).slice();for(var x=u.slice().sort(function(_,J){return J.y-_.y||_.x-J.x}),y=[],C=0;C<x.length;C++){for(var k=!1,R=0;R<y.length;R++)if(v(y[R][0],x[C])){y[R].push(x[C]),k=!0;break}k||y.push([x[C]])}for(var U=[],j=0;j<y.length;j++){for(var $=y[j].slice().sort(function(_,J){return _.x-J.x}),L=null,X=0;X<$.length;X++){var F=$[X];if(L===null){L={x:F.x,y:F.y,width:F.width,height:F.height};continue}var re=F.x-(L.x+L.width);if(re<=w*Math.max(L.height,F.height)){var te=Math.max(L.x+L.width,F.x+F.width),ve=Math.max(L.y+L.height,F.y+F.height);L.x=Math.min(L.x,F.x),L.y=Math.min(L.y,F.y),L.width=te-L.x,L.height=ve-L.y}else U.push(L),L={x:F.x,y:F.y,width:F.width,height:F.height}}L!==null&&U.push(L)}return U.map(function(_){return a(_)})}function S(u,f,w,x){var y=x===void 0?0:x;return f>=u.x-y&&f<=u.x+u.width+y&&w>=u.y-y&&w<=u.y+u.height+y}function N(u,f,w,x,y){for(var C=u||[],k=C.length-1;k>=0;k--){var R=C[k];if(!(!R||R.page!==f||!R.rects)){for(var U=0;U<R.rects.length;U++)if(S(R.rects[U],w,x,y===void 0?1:y))return R}}return null}function E(u){return String(u??"").replace(/\s+/g," ").trim()}return{clientRectToLocal:t,rectFromCorners:s,roundRect:a,isVisibleRect:l,textTokenRanges:r,unionClientRects:d,clientRectsToPdfRects:o,pdfRectToViewportRect:h,itemRelativeRect:b,mergeLineRects:I,rectContainsPoint:S,hitTestHighlights:N,normalizeQuoteText:E}}var B=Ie(),Yn=B.clientRectToLocal,Xn=B.rectFromCorners,Jn=B.roundRect,Qn=B.isVisibleRect,Kn=B.textTokenRanges,Zn=B.unionClientRects,eo=B.clientRectsToPdfRects,to=B.pdfRectToViewportRect,no=B.itemRelativeRect,oo=B.mergeLineRects,ao=B.rectContainsPoint,ro=B.hitTestHighlights,io=B.normalizeQuoteText;function ke(){var t=[.957,.871,.424];function s(d,o,h,b,v){var I=o.context.register(o.context.obj({Type:d.PDFName.of("ExtGState"),BM:d.PDFName.of("Multiply"),ca:d.PDFNumber.of(.4)})),S=[d.pushGraphicsState(),d.setGraphicsState("GS0")];S.push(d.setFillingColor(d.rgb(b[0],b[1],b[2])));for(var N=0;N<h.length;N++){var E=h[N];S.push(d.moveTo(E.x,E.y)),S.push(d.lineTo(E.x,E.y+E.height)),S.push(d.lineTo(E.x+E.width,E.y+E.height)),S.push(d.lineTo(E.x+E.width,E.y)),S.push(d.closePath())}S.push(d.fill()),S.push(d.popGraphicsState());var u=o.context.formXObject(S,{BBox:v,Resources:{ExtGState:{GS0:I}}});return o.context.register(u)}function a(d,o,h,b){for(var v=h.rects,I=[],S=v[0].x,N=v[0].y,E=v[0].x+v[0].width,u=v[0].y+v[0].height,f=0;f<v.length;f++){var w=v[f],x=w.x,y=w.x+w.width,C=w.y,k=w.y+w.height;I.push(x,k,y,k,x,C,y,C),S=Math.min(S,x),N=Math.min(N,C),E=Math.max(E,y),u=Math.max(u,k)}var R=o.context.obj({Type:d.PDFName.of("Annot"),Subtype:d.PDFName.of("Highlight"),Rect:o.context.obj([S,N,E,u]),QuadPoints:o.context.obj(I),C:o.context.obj(b),F:d.PDFNumber.of(4),T:d.PDFString.of("PDF Annotator"),M:d.PDFString.of(new Date().toISOString()),CA:d.PDFNumber.of(.4)});h.note&&R.set(d.PDFName.of("Contents"),d.PDFString.of(h.note));var U=s(d,o,v,b,[S,N,E,u]);R.set(d.PDFName.of("AP"),o.context.obj({N:U}));var j=o.context.register(R),$=[j];if(h.note){var L=o.context.register(o.context.obj({Type:d.PDFName.of("Annot"),Subtype:d.PDFName.of("Popup"),Rect:o.context.obj([E+8,N-60,E+208,N+12]),Parent:j,Open:!1}));R.set(d.PDFName.of("Popup"),L),$.push(L)}return $}function l(d,o,h){var b=o.node.get(d.PDFName.of("Annots"));if(b instanceof d.PDFArray)for(var v=0;v<h.length;v++)b.push(h[v]);else o.node.set(d.PDFName.of("Annots"),o.doc.context.obj(h))}async function r(d,o,h,b){for(var v=await d.PDFDocument.load(o),I=v.getPages(),S=h||[],N=0;N<S.length;N++){var E=S[N];if(!(!E||!E.rects||!E.rects.length)){var u=I[E.page-1];if(u){var f=b&&b[E.color]||t,w=a(d,v,E,f);l(d,u,w)}}}return v.save()}return{writeHighlightsIntoPdf:r,buildHighlightAnnotation:a,appendAnnotationRefs:l}}var Pe=ke(),lo=Pe.writeHighlightsIntoPdf,co=Pe.buildHighlightAnnotation,ho=Pe.appendAnnotationRefs;function De(){function t(o){return String(o??"").replace(/\]/g,"\\]")}function s(o,h,b,v,I){var S=new URLSearchParams;h&&S.set("att",h),Number.isFinite(b)&&b>=1&&S.set("page",String(Math.floor(b))),v&&S.set("hl",v),I&&S.set("note",I);var N=S.toString();return"plugin://"+o+(N?"?"+N:"")}function a(o,h){return String(o??"").split(/\r?\n/).map(function(b){return(h+" "+b).replace(/[ \t]+$/,"")})}function l(o,h,b,v,I,S){var N=s(h,b,v.page,v.id,S),E=t(o||"PDF"),u='==\u25CF<!-- {"cycleColor":"'+I+'"} -->==',f=u+" ["+E+"]("+N+")",w=[f].concat(a(v.quoteText,"> >"));return v.note&&(w.push(">"),w=w.concat(a(v.note,">"))),w.join(`
`)}function r(o){return o.slice().sort(function(h,b){if(h.page!==b.page)return h.page-b.page;var v=h.rects&&h.rects[0]?h.rects[0].y:0,I=b.rects&&b.rects[0]?b.rects[0].y:0;return I-v})}function d(o,h,b,v,I,S,N){var E=S&&S.length?S:null,u=(v||[]).filter(function(x){return x&&(!E||E.indexOf(x.color)!==-1)}),f=r(u),w=f.map(function(x){var y=I?I[x.color]:void 0;return l(o,h,b,x,y,N)});return w.join(`

`)}return{buildDeepLink:s,buildHighlightBlock:l,buildExportAllContent:d}}var Ue=De(),uo=Ue.buildDeepLink,fo=Ue.buildHighlightBlock,go=Ue.buildExportAllContent;function bt(){var t=window.__PDFA_CONFIG||{},s=window.__PDFA_GEOM||{},a=window.__PDFA_ANNOTATIONS||{},l=window.__PDFA_EXPORT||{},r={root:document.getElementById("pdfa-root"),pages:document.getElementById("pdfa-pages"),status:document.getElementById("pdfa-status"),pageLabel:document.getElementById("pdfa-page-label"),zoomLabel:document.getElementById("pdfa-zoom-label"),colors:document.getElementById("pdfa-colors"),hint:document.getElementById("pdfa-hint"),popover:document.getElementById("pdfa-popover"),panel:document.getElementById("pdfa-panel"),listToggle:document.getElementById("pdfa-list-toggle"),count:document.getElementById("pdfa-count"),more:document.getElementById("pdfa-more"),open:document.getElementById("pdfa-open"),scrollUp:document.getElementById("pdfa-scroll-up"),scrollDown:document.getElementById("pdfa-scroll-down"),collapsedCount:document.getElementById("pdfa-collapsed-count"),collapsedName:document.querySelector(".pdfa-collapsed-name"),name:document.querySelector(".pdfa-name")};function d(e){e&&(o.attachmentName=e,r.name&&(r.name.textContent=e),r.collapsedName&&(r.collapsedName.textContent=e))}var o={doc:null,scale:1.25,pageCount:0,current:1,rendering:!1,textSpans:0,viewports:{},rendered:{},renderingPage:{},highlights:[],pdfBytes:null,attachmentName:t.attachmentName||"",activeColorId:t.defaultColorId||((t.colors||[{}])[0]||{}).id,pendingSelection:null,lastCapturedText:"",noteEditing:null};function h(e,n){r.status.textContent=e||"",r.status.style.display=e?"block":"none",r.status.className=n?"pdfa-status pdfa-error":"pdfa-status"}function b(e){var n=Object.assign({noteUUID:t.noteUUID},e);return new Promise(function(i,c){try{if(typeof window.callAmplenotePlugin!="function")throw new Error("Plugin bridge unavailable (callAmplenotePlugin missing)");i(window.callAmplenotePlugin(JSON.stringify(n)))}catch(p){c(p)}}).then(function(i){if(i&&typeof i=="object")return i;if(typeof i!="string")throw new Error("Empty reply from the plugin");try{return JSON.parse(i)}catch{throw new Error("Unreadable reply from the plugin: "+String(i).slice(0,120))}})}function v(){return t.colors||[]}function I(e){for(var n=v(),i=0;i<n.length;i++)if(n[i].id===e)return n[i].hex;return n.length?n[0].hex:"#F4DE6C"}function S(e){for(var n=0;n<o.highlights.length;n++)if(o.highlights[n].id===e)return o.highlights[n];return null}function N(e,n,i){var c=document.createElement("button");return c.className="pdfa-btn"+(n?" "+n:""),c.textContent=e,c.onclick=function(p){p.stopPropagation(),i()},c}function E(e,n,i,c){var p=document.createElement("button");return p.className="pdfa-color",p.dataset.color=e.id,p.style.background=e.hex,p.title=c+" "+e.label,p.setAttribute("aria-label",c+" "+e.label),p.setAttribute("aria-pressed",String(!!n)),p.onclick=function(m){m.stopPropagation(),i(e.id)},p}function u(){for(var e=v(),n=0;n<e.length;n++)r.colors.appendChild(E(e[n],e[n].id===o.activeColorId,function(i){o.activeColorId=i,f(),o.pendingSelection&&Re(o.pendingSelection,i)},"Highlight"))}function f(){for(var e=r.colors.querySelectorAll(".pdfa-color"),n=0;n<e.length;n++)e[n].setAttribute("aria-pressed",String(e[n].dataset.color===o.activeColorId))}function w(){for(var e=[],n=1;n<=o.pageCount;n++)(function(i){e.push(o.doc.getPage(i).then(function(c){o.viewports[i]=c.getViewport({scale:o.scale})}))})(n);return Promise.all(e)}function x(e){var n=o.viewports[e],i=document.createElement("div");return i.className="pdfa-page",i.dataset.page=String(e),i.style.width=n.width+"px",i.style.height=n.height+"px",i}function y(e,n){if(o.rendered[n]||o.renderingPage[n])return Promise.resolve();o.renderingPage[n]=!0;var i=o.viewports[n],c=document.createElement("canvas"),p=window.devicePixelRatio||1;c.width=Math.floor(i.width*p),c.height=Math.floor(i.height*p),c.style.width=i.width+"px",c.style.height=i.height+"px",e.appendChild(c);var m=document.createElement("div");m.className="pdfa-highlights",e.appendChild(m);var g=document.createElement("div");g.className="textLayer",g.style.width=i.width+"px",g.style.height=i.height+"px",g.style.setProperty("--scale-factor",String(o.scale)),e.appendChild(g);var A=c.getContext("2d");A.scale(p,p);var H=null;return o.doc.getPage(n).then(function(P){return H=P,P.render({canvasContext:A,viewport:i}).promise}).then(function(){return H.getTextContent()}).then(function(P){var T=[];return window.pdfjsLib.renderTextLayer({textContent:P,container:g,viewport:i,textDivs:T}).promise.then(function(){o.textSpans+=T.length;for(var D=0;D<T.length;D++)T[D].__pdfaItem=P.items[D];o.rendered[n]=!0,o.renderingPage[n]=!1,j(n),k()})}).catch(function(P){o.renderingPage[n]=!1,h("Failed to render page "+n+": "+(P.message||P),!0)})}function C(){var e=Y();if(!e||!o.doc)return Promise.resolve();for(var n=e.getBoundingClientRect(),i=e.clientHeight,c=r.pages.querySelectorAll(".pdfa-page"),p=[],m=0;m<c.length;m++){var g=c[m],A=Number(g.dataset.page);if(!(o.rendered[A]||o.renderingPage[A])){var H=g.getBoundingClientRect(),P=H.top-n.top,T=H.bottom-n.top;T<-i||P>e.clientHeight+i||p.push(y(g,A))}}return Promise.all(p)}function k(){var e=0;for(var n in o.rendered)o.rendered[n]&&e++;if(e){var i=o.textSpans===0;h(i?"No selectable text found - this PDF may be a scan.":"",i)}}function R(){if(o.rendering)return Promise.resolve();o.rendering=!0,M(!0),h("Rendering...");var e=Y(),n=e?e.scrollHeight-e.clientHeight:0,i=n>0?e.scrollTop/n:0;return r.pages.innerHTML="",o.viewports={},o.rendered={},o.renderingPage={},o.textSpans=0,w().then(function(){for(var c=1;c<=o.pageCount;c++)r.pages.appendChild(x(c));if(e){var p=e.scrollHeight-e.clientHeight;e.scrollTop=i*(p>0?p:0)}o.rendering=!1,ne(),Z(),C()}).catch(function(c){o.rendering=!1,h("Failed to render: "+(c.message||c),!0)})}function U(e){return function(n,i){return e.convertToViewportPoint(n,i)}}function j(e){for(var n=e?'.pdfa-page[data-page="'+e+'"]':".pdfa-page",i=r.pages.querySelectorAll(n),c=0;c<i.length;c++){var p=i[c],m=Number(p.dataset.page),g=p.querySelector(".pdfa-highlights"),A=o.viewports[m];if(!(!g||!A)){g.innerHTML="";for(var H=U(A),P=0;P<o.highlights.length;P++){var T=o.highlights[P];if(!(!T||T.page!==m||!T.rects||!T.rects.length)){var D=document.createElement("div");D.className="pdfa-hl-group",D.dataset.id=T.id||"";for(var z=0;z<T.rects.length;z++){var G=s.pdfRectToViewportRect(T.rects[z],H),O=document.createElement("div");O.className="pdfa-hl",O.style.left=G.x+"px",O.style.top=G.y+"px",O.style.width=G.width+"px",O.style.height=G.height+"px",O.style.background=I(T.color),D.appendChild(O)}g.appendChild(D)}}}}}function $(){j(),X(),r.count.textContent=String(o.highlights.length)}function L(){return o.highlights.slice().sort(function(e,n){return e.page!==n.page?e.page-n.page:(n.rects[0]?n.rects[0].y:0)-(e.rects[0]?e.rects[0].y:0)})}function X(){r.panel.innerHTML="";var e=document.createElement("div");e.className="pdfa-panel-title";var n=document.createElement("span");n.textContent="Highlights",e.appendChild(n),e.appendChild(N("Close","",function(){re(!1)})),r.panel.appendChild(e);var i=L();if(!i.length){var c=document.createElement("div");c.className="pdfa-panel-empty",c.textContent="No highlights yet. Select some text in the PDF and pick a color.",r.panel.appendChild(c);return}for(var p=0;p<i.length;p++)r.panel.appendChild(F(i[p]))}function F(e){var n=document.createElement("div");n.className="pdfa-hl-row",n.dataset.id=e.id||"",n.title="Jump to this highlight";var i=document.createElement("span");i.className="pdfa-chip",i.style.background=I(e.color),n.appendChild(i);var c=document.createElement("div"),p=document.createElement("div");p.className="pdfa-hl-page",p.textContent="Page "+e.page,c.appendChild(p);var m=document.createElement("div");if(m.className="pdfa-hl-quote",m.textContent=e.quoteText.length>160?e.quoteText.slice(0,160)+"...":e.quoteText,c.appendChild(m),e.note){var g=document.createElement("div");g.className="pdfa-hl-note",g.textContent=e.note,c.appendChild(g)}return n.appendChild(c),n.onclick=function(){Oe(e)},n}function re(e){var n=e===void 0?!r.panel.classList.contains("pdfa-open"):e;r.panel.classList.toggle("pdfa-open",n),r.listToggle.setAttribute("aria-pressed",String(n)),n&&X(),Z()}function te(e){for(var n=e&&e.nodeType===1?e:e&&e.parentElement;n;){if(n.classList&&n.classList.contains("textLayer"))return n;n=n.parentElement}return null}function ve(e,n){for(var i=[],c=[],p=null,m=document.createTreeWalker(n,NodeFilter.SHOW_TEXT,null),g;g=m.nextNode();)if(e.intersectsNode(g)){var A=g.nodeValue||"",H=g===e.startContainer?e.startOffset:0,P=g===e.endContainer?e.endOffset:A.length,T=g.parentElement,D=T&&T.__pdfaItem;if(D)for(var z={x1:D.transform[4],y1:D.transform[5],x2:D.transform[4]+D.width,y2:D.transform[5]+D.height},G=T.getBoundingClientRect(),O=s.textTokenRanges(A,H,P),ee=0;ee<O.length;ee++){var ye=document.createRange();ye.setStart(g,O[ee].start),ye.setEnd(g,O[ee].end);var V=s.unionClientRects(ye.getClientRects());if(V){var We={left:V.left,top:V.top,width:V.width,height:V.height,right:V.left+V.width,bottom:V.top+V.height},Ye=s.itemRelativeRect(z,G,We);Ye&&(i.push(Ye),c.push(A.slice(O[ee].start,O[ee].end)),p=We)}}}return{rects:i,text:c.join(" "),lastCssRect:p}}function _(e){if(o.pendingSelection=e,o.lastCapturedText=e&&e.rawText||"",!e){r.hint.textContent="",r.hint.style.display="none";return}r.hint.textContent=e.spilled?"Pick a color (page "+e.page+" only)":"Pick a color",r.hint.style.display="inline"}function J(e){if(!o.noteEditing){var n=window.getSelection();if(!n||n.isCollapsed||n.rangeCount===0){_(null),M();return}var i=n.getRangeAt(0),c=te(i.startContainer);if(!c)return _(null);var p=c.parentElement;if(!p||!p.dataset||!p.dataset.page)return _(null);var m=Number(p.dataset.page);if(!o.rendered[m])return _(null);var g=te(i.endContainer)!==c,A=ve(i,c),H=s.mergeLineRects(A.rects);if(!H.length)return _(null);var P=A.lastCssRect||p.getBoundingClientRect(),T=e&&e.clientX?e.clientX:P.left+P.width/2,D=e&&e.clientY?e.clientY:P.top+P.height,z={page:m,rects:H,quoteText:s.normalizeQuoteText(A.text),spilled:g,anchorX:T,anchorY:D,rawText:String(n)};_(z),At(z)}}var Et=300,Q=null;function Nt(){o.noteEditing||(Q&&clearTimeout(Q),Q=setTimeout(He,Et))}function He(){if(Q=null,!o.noteEditing){var e=window.getSelection();!e||e.isCollapsed||e.rangeCount===0||te(e.getRangeAt(0).startContainer)&&String(e)!==o.lastCapturedText&&J(null)}}function ie(e,n){var i=o.highlights;return o.highlights=e,$(),b(n).then(function(c){if(!c||c.error)throw new Error(c&&c.error||"The plugin did not confirm the change.");return o.highlights=c.highlights||e,$(),h(""),!0}).catch(function(c){return o.highlights=i,$(),h(c.message||String(c),!0),!1})}function Re(e,n){var i={id:null,page:e.page,color:n,rects:e.rects,quoteText:e.quoteText,note:null},c=e.anchorX,p=e.anchorY;_(null),M(!0);var m=window.getSelection();m&&m.removeAllRanges&&m.removeAllRanges(),ie(o.highlights.concat([i]),{action:"addHighlight",attachmentUUID:t.attachmentUUID,highlight:i}).then(function(g){if(g){var A=o.highlights[o.highlights.length-1];A&&A.id&&xe(A,c,p,!0)}})}function St(e,n){M(!0),ie(o.highlights.map(function(i){return i.id===e?Object.assign({},i,{color:n}):i}),{action:"recolorHighlight",attachmentUUID:t.attachmentUUID,id:e,color:n})}function Tt(e){M(!0),ie(o.highlights.filter(function(n){return n.id!==e}),{action:"removeHighlight",attachmentUUID:t.attachmentUUID,id:e})}function we(e,n){var i=String(n??"").trim();o.noteEditing=null,M(!0),ie(o.highlights.map(function(c){return c.id===e?Object.assign({},c,{note:i||null}):c}),{action:"setHighlightNote",attachmentUUID:t.attachmentUUID,id:e,note:i})}function K(e,n,i,c){r.popover.innerHTML="",r.popover.classList.toggle("pdfa-editing",c==="editing"),r.popover.classList.toggle("pdfa-exporting",c==="exporting"),r.popover.classList.toggle("pdfa-menu",c==="menu");for(var p=0;p<e.length;p++)r.popover.appendChild(e[p]);r.popover.classList.add("pdfa-open");var m=r.popover.offsetWidth,g=r.popover.offsetHeight,A=Math.max(4,Math.min(n-m/2,window.innerWidth-m-4)),H=i+12;H+g>window.innerHeight-4&&(H=Math.max(4,i-g-12)),r.popover.style.left=A+"px",r.popover.style.top=H+"px"}function M(e){o.noteEditing&&!e||(o.noteEditing=null,r.popover.classList.remove("pdfa-open","pdfa-editing","pdfa-exporting","pdfa-menu"),r.popover.innerHTML="")}function At(e){for(var n=v(),i=[],c=0;c<n.length;c++)i.push(E(n[c],n[c].id===o.activeColorId,function(p){o.activeColorId=p,f(),Re(e,p)},"Highlight"));K(i,e.anchorX,e.anchorY)}function xe(e,n,i,c){for(var p=v(),m=[],g=0;g<p.length;g++)m.push(E(p[g],p[g].id===e.color,function(H){St(e.id,H)},"Change to"));var A=!!e.note;m.push(N(A?"Edit note":"Add note",c&&!A?"pdfa-btn-primary":"",function(){kt(e,n,i)})),m.push(N("Copy","",function(){jt(e)})),m.push(N("Send to note","",function(){_t(e)})),m.push(N("Remove","pdfa-remove",function(){Tt(e.id)})),K(m,n,i)}function It(e,n){for(var i=v(),c={},p=0;p<i.length;p++)c[i[p].id]=!0;var m=document.createElement("div");m.className="pdfa-export-hint",m.textContent="Export highlights to a note";var g=document.createElement("div");g.className="pdfa-export-colors";for(var A=0;A<i.length;A++)(function(P){var T=E(P,!0,function(D){c[D]=!c[D],T.setAttribute("aria-pressed",String(c[D]))},"Toggle");g.appendChild(T)})(i[A]);var H=document.createElement("div");H.className="pdfa-note-actions",H.appendChild(N("Create / update note","pdfa-btn-primary",function(){for(var P=[],T=0;T<i.length;T++)c[i[T].id]&&P.push(i[T].id);Bt(P.length===i.length?null:P)})),K([m,g,H],e,n,"exporting")}function kt(e,n,i){o.noteEditing=e.id;var c=document.createElement("textarea");c.className="pdfa-note-input",c.rows=3,c.value=e.note||"",c.placeholder="Note for this highlight";var p=document.createElement("div");p.className="pdfa-note-actions",e.note&&p.appendChild(N("Delete note","",function(){we(e.id,"")}));var m=document.createElement("span");m.className="pdfa-spacer",p.appendChild(m),p.appendChild(N("Cancel","",function(){Le(e,n,i)})),p.appendChild(N("Save","pdfa-btn-primary",function(){we(e.id,c.value)})),c.onkeydown=function(g){g.key==="Enter"&&(g.ctrlKey||g.metaKey)?(g.preventDefault(),g.stopPropagation(),we(e.id,c.value)):g.key==="Escape"&&(g.preventDefault(),g.stopPropagation(),Le(e,n,i))},K([c,p],n,i,"editing"),c.focus(),c.setSelectionRange(c.value.length,c.value.length)}function Le(e,n,i){o.noteEditing=null;var c=S(e.id)||e;xe(c,n,i)}function Pt(e){if(!o.noteEditing){var n=window.getSelection();if(!(n&&!n.isCollapsed)){for(var i=e.target,c=null;i&&i!==r.pages;){if(i.classList&&i.classList.contains("pdfa-page")){c=i;break}i=i.parentElement}if(!c)return M();var p=Number(c.dataset.page),m=o.viewports[p];if(!m)return M();var g=c.getBoundingClientRect(),A=m.convertToPdfPoint(e.clientX-g.left,e.clientY-g.top),H=s.hitTestHighlights(o.highlights,p,A[0],A[1],1);H&&H.id?xe(H,e.clientX,e.clientY):M()}}}function ne(){r.pageLabel.textContent=o.current+" / "+o.pageCount,r.zoomLabel.textContent=Math.round(o.scale*100)+"%"}function Y(){return r.root.querySelector(".pdfa-scroll")}function Me(){return r.panel&&r.panel.classList.contains("pdfa-open")?r.panel:Y()}function Fe(e){var n=r.pages.querySelector('.pdfa-page[data-page="'+e+'"]');n&&y(n,e)}function be(e){var n=Math.min(Math.max(1,e),o.pageCount),i=r.pages.querySelector('.pdfa-page[data-page="'+n+'"]');Fe(n);var c=Y();i&&c&&(c.scrollTop+=i.getBoundingClientRect().top-c.getBoundingClientRect().top),C(),o.current=n,ne()}function Oe(e){var n=r.pages.querySelector('.pdfa-page[data-page="'+e.page+'"]'),i=o.viewports[e.page];if(!(!n||!i||!e.rects||!e.rects.length)){var c=s.pdfRectToViewportRect(e.rects[0],U(i)),p=Y(),m=n.getBoundingClientRect().top+c.y;p.scrollTop+=m-p.getBoundingClientRect().top-p.clientHeight/3,Fe(e.page),C(),o.current=e.page,ne()}}function Dt(){try{r.root.setAttribute("tabindex","-1"),r.root.focus(),r.root.scrollIntoView&&r.root.scrollIntoView({block:"nearest"})}catch{}}function Ut(e){if(!(!e||!e.id)){var n=r.pages.querySelector('.pdfa-hl-group[data-id="'+e.id+'"]');n&&(n.classList.add("pdfa-hl-flash"),setTimeout(function(){n.classList.remove("pdfa-hl-flash")},2600))}}function $e(e){return o.scale=Math.min(Math.max(.4,e),4),R()}function Ht(){return o.doc?o.doc.getPage(1).then(function(e){var n=Y();if(n){var i=window.getComputedStyle(n),c=n.clientWidth-(parseFloat(i.paddingLeft)||0)-(parseFloat(i.paddingRight)||0),p=e.getViewport({scale:1}).width;if(!(!(c>0)||!(p>0))){var m=Math.max(.4,c/p);m<o.scale&&(o.scale=m,ne())}}}).catch(function(){}):Promise.resolve()}function je(e){var n=0,i=0,c=0,p=0,m=!1,g=!1,A=6,H=300;e.addEventListener("touchstart",function(T){m=T.touches.length!==1,g=!1,!m&&(n=T.touches[0].clientX,i=T.touches[0].clientY,c=i,p=Date.now())},{passive:!0}),e.addEventListener("touchmove",function(T){if(T.touches.length===1){var D=T.touches[0];if(!m){var z=D.clientX-n,G=D.clientY-i;if(Math.abs(z)<A&&Math.abs(G)<A)return;m=!0;var O=window.getSelection();g=Math.abs(G)>Math.abs(z)&&Date.now()-p<=H&&!(O&&!O.isCollapsed),c=D.clientY}g&&(T.preventDefault(),e.scrollTop-=D.clientY-c,c=D.clientY,Z(),C())}},{passive:!1});var P=function(){m=!1,g=!1};e.addEventListener("touchend",P,{passive:!0}),e.addEventListener("touchcancel",P,{passive:!0})}function _e(e){var n=Me();n&&(n.scrollTop+=e*Math.max(80,n.clientHeight*.85),Z(),C())}function Z(){var e=Me();if(!(!e||!r.scrollUp)){var n=e.scrollHeight-e.clientHeight;r.scrollUp.disabled=e.scrollTop<=1,r.scrollDown.disabled=e.scrollTop>=n-1}}function Rt(){Z(),C(),M();for(var e=r.pages.querySelectorAll(".pdfa-page"),n=o.current,i=1/0,c=0;c<e.length;c++){var p=Math.abs(e[c].getBoundingClientRect().top-60);p<i&&(i=p,n=Number(e[c].dataset.page))}n!==o.current&&(o.current=n,ne())}function Lt(){return new Promise(function(e,n){if(window.pdfjsLib)return e(window.pdfjsLib);var i=document.createElement("script");i.src=t.pdfJsSrc,i.onload=function(){window.pdfjsLib?e(window.pdfjsLib):n(new Error("PDF.js loaded but did not register itself."))},i.onerror=function(){n(new Error("Could not load PDF.js from the CDN."))},document.head.appendChild(i)})}function Mt(){return new Promise(function(e,n){if(window.PDFLib)return e(window.PDFLib);var i=document.createElement("script");i.src=t.pdfLibSrc,i.onload=function(){window.PDFLib?e(window.PDFLib):n(new Error("pdf-lib loaded but did not register itself."))},i.onerror=function(){n(new Error("Could not load pdf-lib from the CDN."))},document.head.appendChild(i)})}function Ft(){for(var e={},n=v(),i=0;i<n.length;i++)n[i].rgb&&(e[n[i].id]=n[i].rgb);return e}function Ot(){var e=(o.attachmentName||"annotated").replace(/\.pdf$/i,"");return e+"-annotated.pdf"}function Be(){for(var e={},n=v(),i=0;i<n.length;i++)n[i].cycleIndex!==void 0&&(e[n[i].id]=n[i].cycleIndex);return e}function ze(){var e=(o.attachmentName||"PDF").replace(/\.pdf$/i,"");return e+" - Highlights"}function qe(e){return l.buildHighlightBlock(o.attachmentName,t.pluginUUID,t.attachmentUUID,e,Be()[e.color],t.noteUUID)}function $t(e){return navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(e):new Promise(function(n,i){var c=document.createElement("textarea");c.value=e,c.style.position="fixed",c.style.left="-9999px",document.body.appendChild(c),c.focus(),c.select();var p=!1;try{p=document.execCommand("copy")}catch{p=!1}document.body.removeChild(c),p?n():i(new Error("Clipboard access is unavailable here."))})}function jt(e){M(!0),$t(qe(e)).then(function(){h("Highlight copied - paste it into any note.")}).catch(function(n){h("Could not copy: "+(n.message||n),!0)})}function _t(e){M(!0),b({action:"sendToNote",content:qe(e)}).then(function(n){if(!n||n.error)throw new Error(n&&n.error||"Could not send this to the note.");h("Sent to the bottom of this note.")}).catch(function(n){h(n.message||String(n),!0)})}function Bt(e){M(!0);var n=l.buildExportAllContent(o.attachmentName,t.pluginUUID,t.attachmentUUID,o.highlights,Be(),e,t.noteUUID);if(!n){h(e?"No highlights match those colors.":"No highlights to export yet.",!0);return}b({action:"exportAll",noteName:ze(),content:n}).then(function(i){if(!i||i.error)throw new Error(i&&i.error||"Could not export highlights.");h('Exported to "'+ze()+'".')}).catch(function(i){h(i.message||String(i),!0)})}function zt(e,n){var i=document.createElement("div");i.className="pdfa-menu-name",i.textContent=o.attachmentName||"PDF Annotator",i.title=i.textContent;var c=[i];c.push(N("Collapse","",function(){M(!0),Yt()}),N("Download","",function(){M(!0),Vt()}),N("Export...","",function(){It(e,n)}),N("Remove viewer...","pdfa-remove",function(){qt(e,n)})),K(c,e,n,"menu")}function qt(e,n){var i=document.createElement("div");i.className="pdfa-export-hint",i.textContent="Remove this viewer and all its highlights from this note? This cannot be undone.";var c=document.createElement("div");c.className="pdfa-note-actions",c.appendChild(N("Cancel","",function(){M(!0)}));var p=document.createElement("span");p.className="pdfa-spacer",c.appendChild(p),c.appendChild(N("Remove","pdfa-remove",Gt)),K([i,c],e,n,"exporting")}function Gt(){M(!0),h("Removing this viewer..."),b({action:"removeViewer",attachmentUUID:t.attachmentUUID,pluginUUID:t.pluginUUID}).then(function(e){if(!e||e.error)throw new Error(e&&e.error||"Could not remove this viewer.");document.body.innerHTML='<div style="padding:16px;font:13px sans-serif;opacity:.75">Removed - this block will disappear once the note refreshes.</div>'}).catch(function(e){h(e.message||String(e),!0)})}function Vt(){o.pdfBytes&&(h("Preparing the download..."),Mt().then(function(e){return a.writeHighlightsIntoPdf(e,o.pdfBytes,o.highlights,Ft())}).then(function(e){var n=new Blob([e],{type:"application/pdf"}),i=URL.createObjectURL(n),c=document.createElement("a");c.href=i,c.download=Ot(),document.body.appendChild(c),c.click(),c.remove(),setTimeout(function(){URL.revokeObjectURL(i)},4e3),h("")}).catch(function(e){h("Could not prepare the download: "+(e.message||e),!0)}))}function Wt(){return b({action:"loadHighlights",attachmentUUID:t.attachmentUUID}).then(function(e){if(!e||e.error)throw new Error(e&&e.error||"No answer from the plugin");o.highlights=e.highlights||[]}).catch(function(e){o.highlights=[],h("Could not load saved highlights: "+(e.message||e),!0)})}function Yt(){var e=o.highlights.length;r.collapsedCount.textContent=e?e+(e===1?" highlight":" highlights"):"",r.root.classList.add("pdfa-collapsed-mode"),Ge(!0)}function Ge(e){b({action:"setCollapsed",collapsed:e,attachmentUUID:t.attachmentUUID,pluginUUID:t.pluginUUID}).catch(function(){})}function Xt(){r.root.classList.remove("pdfa-collapsed-mode"),o.doc||Ve(),Ge(!1)}function Ve(){h("Loading PDF..."),(t.highlightId||t.page)&&Dt(),Lt().then(function(e){return e.GlobalWorkerOptions.workerSrc=t.workerSrc,b({action:"getPdfUrl",attachmentUUID:t.attachmentUUID})}).then(function(e){if(!e||!e.url)throw new Error(e&&e.error||"Could not resolve the PDF URL");return d(e.name),fetch(e.url)}).then(function(e){if(!e.ok)throw new Error("Download failed (HTTP "+e.status+")");return e.arrayBuffer()}).then(function(e){return o.pdfBytes=e.slice(0),window.pdfjsLib.getDocument({data:e}).promise}).then(function(e){return o.doc=e,o.pageCount=e.numPages,Wt()}).then(function(){return Ht()}).then(function(){return R()}).then(function(){$();var e=t.highlightId?S(t.highlightId):null;e?(Oe(e),Ut(e)):t.page&&be(t.page)}).catch(function(e){h(e.message||String(e),!0)})}try{document.getElementById("pdfa-prev").onclick=function(){be(o.current-1)},document.getElementById("pdfa-next").onclick=function(){be(o.current+1)},document.getElementById("pdfa-zoom-in").onclick=function(){$e(o.scale+.25)},document.getElementById("pdfa-zoom-out").onclick=function(){$e(o.scale-.25)},r.scrollUp.onclick=function(){_e(-1)},r.scrollDown.onclick=function(){_e(1)},r.listToggle.onclick=function(){re()},r.more.onclick=function(e){zt(e.clientX,e.clientY)},Y().addEventListener("scroll",Rt),r.panel.addEventListener("scroll",Z),je(Y()),je(r.panel),r.pages.addEventListener("mouseup",J),r.pages.addEventListener("click",Pt),document.addEventListener("selectionchange",Nt),r.pages.addEventListener("touchend",function(){Q&&clearTimeout(Q),Q=null,He()}),document.addEventListener("keydown",function(e){e.key==="Escape"&&!o.noteEditing&&M()}),document.addEventListener("mousedown",function(e){r.popover.classList.contains("pdfa-open")&&(r.popover.contains(e.target)||M())}),u(),X(),r.root.querySelector(".pdfa-collapsed").onclick=Xt,t.collapsed?b({action:"getViewerSummary",attachmentUUID:t.attachmentUUID}).then(function(e){if(!(!e||e.error)){d(e.name);var n=e.count||0;r.collapsedCount.textContent=n?n+(n===1?" highlight":" highlights"):""}}).catch(function(){}):Ve()}catch(e){h("Viewer failed to start: "+(e&&e.message?e.message:e),!0)}}function gn(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function mn(t){return JSON.stringify(t).replace(/</g,"\\u003c")}var vn=`
