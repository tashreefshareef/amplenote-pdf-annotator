...
`)}function Ne(t,l,a,s={}){if(!t||!l||!a)return null;let i=t.split(`
`),d=`plugin://${l}`,o=i.findIndex(T=>T.includes(d)&&T.includes(`att=${a}`));if(o===-1)return null;let p=i[o],A=p.match(/data="(plugin:\/\/[^"]*)"/);if(!A)return null;let g=A[1],S=g.indexOf("?"),N=S===-1?"":g.slice(S+1),C={...ae(N),attachmentUUID:a,...s},u=nt(C),f=u?`plugin://${l}?${u}`:`plugin://${l}`,x=i.slice(),b=p.replace(A[0],`data="${f}"`),E=tt(C.collapsed),y=b.match(/data-aspect-ratio="[^"]*"/);return b=y?b.replace(y[0],`data-aspect-ratio="${E}"`):b.replace(/\s*\/>\s*$/,` data-aspect-ratio="${E}" />`),x[o]=b,x.join(`
`)}function at(t,l,a,s){return Ne(t,l,a,{collapsed:!!s})}async function rt(t,l,a){let s=await ce(t,l);if(!s){let p=await t.getNoteAttachments({uuid:l});return(!(Array.isArray(p)&&p.length>0)||!p.some(g=>g&&g.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.
...
`),s=Ae(a,G);if(!s)return null;let i=a.slice(0,s.start).join(`
...
`),s=Ae(a,G);if(!s)return null;let i=mn(a.slice(s.start+1,s.end).join(`
...
`,Ct={light:"--pdfa-bg:#f6f7f9; --pdfa-fg:#1c1e21; --pdfa-toolbar:#fff; --pdfa-border:#d8dbe0; --pdfa-btn:#fff; --pdfa-btn-hover:#eceef1; --pdfa-error:#b3261e; --pdfa-accent:#1a6fb5;",dark:"--pdfa-bg:#1e2126; --pdfa-fg:#e6e8ea; --pdfa-toolbar:#252930; --pdfa-border:#3a3f47; --pdfa-btn:#2d323a; --pdfa-btn-hover:#3a4049; --pdfa-error:#f2b8b5; --pdfa-accent:#79b8ef;"};function Et({attachmentUUID:t,attachmentName:l="",page:a=null,highlightId:s=null,lightDarkMode:i="light",pluginUUID:d=null,noteUUID:o=null,collapsed:p=!1}={}){let A=Ct[i]||Ct.light,g={attachmentUUID:t,page:a,highlightId:s,pluginUUID:d,noteUUID:o,pdfJsSrc:oe.pdfJs,workerSrc:oe.pdfJsWorker,pdfLibSrc:oe.pdfLib,colors:le.map(S=>({id:S.id,label:S.label,hex:S.hex,rgb:S.rgb,cycleIndex:S.cycleIndex})),defaultColorId:se,collapsed:p,attachmentName:l};return`<link rel="stylesheet" href="${oe.pdfViewerCss}">
...
<script>(${yt.toString()})();<\/script>`}var Cn={noteOption:{"Annotate PDF":async function(t,l){return rt(t,l,t.context.pluginUUID)},"Debug: dump note markdown":async function(t,l){return it(t,l)}},insertText:async function(t){return lt(t,t.context.noteUUID,t.context.pluginUUID)},linkTarget:async function(t,...l){return st(t,l[0])},renderEmbed:function(t,...l){let{attachmentUUID:a,page:s,highlightId:i,collapsed:d,attachmentName:o}=ae(l[0]);return a?Et({attachmentUUID:a,page:s,highlightId:i,collapsed:d,attachmentName:o,lightDarkMode:t.context.lightDarkMode,pluginUUID:t.context.pluginUUID,noteUUID:t.context.noteUUID}):`<p style="font:13px sans-serif;padding:12px">
