...
var __pluginModule=(()=>{var Ce=Object.defineProperty;var Jt=Object.getOwnPropertyDescriptor;var Xt=Object.getOwnPropertyNames;var Yt=Object.prototype.hasOwnProperty;var Qt=(t,s)=>{for(var a in s)Ce(t,a,{get:s[a],enumerable:!0})},Kt=(t,s,a,l)=>{if(s&&typeof s=="object"||typeof s=="function")for(let r of Xt(s))!Yt.call(t,r)&&r!==a&&Ce(t,r,{get:()=>s[r],enumerable:!(l=Jt(s,r))||l.enumerable});return t};var Zt=t=>Kt(Ce({},"__esModule",{value:!0}),t);var wn={};Qt(wn,{default:()=>vn});var ie=[{id:"coral",label:"Coral",hex:"#F3998C",cycleIndex:12,rgb:[.953,.6,.549]},{id:"yellow",label:"Yellow",hex:"#F4DE6C",cycleIndex:14,rgb:[.957,.871,.424]},{id:"green",label:"Green",hex:"#BBE077",cycleIndex:15,rgb:[.733,.878,.467]},{id:"blue",label:"Blue",hex:"#84B6D9",cycleIndex:18,rgb:[.518,.714,.851]}],se="yellow",G="PDF Annotator data",We="attachment://",Je=1,Xe=16,ne={pdfJs:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js",pdfJsWorker:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js",pdfViewerCss:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf_viewer.min.css",pdfLib:"https://cdnjs.cloudflare.com/ajax/libs/pdf-lib/1.17.1/pdf-lib.min.js"},en="https://plugins.amplenote.com/cors-proxy";function Ye(t){let s=new URL(en);return s.searchParams.set("apiurl",t),s.toString()}var tn="application/pdf";function nn(t){return Array.isArray(t)?t.filter(s=>s&&s.type===tn&&s.uuid):[]}async function le(t,s){let a=await t.getNoteAttachments({uuid:s}),l=nn(a);if(l.length===0)return null;if(l.length===1)return l[0];let r=await t.prompt("Which PDF do you want to annotate?",{inputs:[{label:"PDF",type:"radio",options:l.map(o=>({label:o.name,value:o.uuid})),value:l[0].uuid}]});if(r==null)return null;let d=Array.isArray(r)?r[0]:r;return l.find(o=>o.uuid===d)||null}async function Qe(t,s){if(!s)throw new Error("fetchableAttachmentURL: attachmentUUID required");let a=await t.getAttachmentURL(s);if(!a)throw new Error(`No URL returned for attachment ${s}`);return Ye(a)}function Ke(t){return t?Xe:Je}function oe(t){let s={attachmentUUID:null,page:null,x:null,y:null,highlightId:null,noteUUID:null,collapsed:!1,attachmentName:""};if(!t||typeof t!="string")return s;let a;try{a=new URLSearchParams(t.replace(/^\?/,""))}catch{return s}let l=d=>{let o=a.get(d);if(o===null||o.trim()==="")return null;let h=Number(o);return Number.isFinite(h)?h:null},r=l("page");return{attachmentUUID:a.get("att")||null,page:r!==null&&r>=1?Math.floor(r):null,x:l("x"),y:l("y"),highlightId:a.get("hl")||null,noteUUID:a.get("note")||null,collapsed:a.get("c")==="1",attachmentName:a.get("n")||""}}function Ze({attachmentUUID:t,page:s,x:a,y:l,highlightId:r,collapsed:d,attachmentName:o}={}){let h=new URLSearchParams;return t&&h.set("att",t),d&&h.set("c","1"),o&&h.set("n",o),Number.isFinite(s)&&s>=1&&h.set("page",String(Math.floor(s))),Number.isFinite(a)&&h.set("x",String(a)),Number.isFinite(l)&&h.set("y",String(l)),r&&h.set("hl",r),h.toString()}function ce(t,s={},a=null){if(!t)throw new Error("buildEmbedMarkup: pluginUUID required");a===null&&(a=Ke(s.collapsed));let l=Ze(s);return`<object data="${l?`plugin://${t}?${l}`:`plugin://${t}`}" data-aspect-ratio="${a}" />`}function et(t,s,a){if(!t||!s||!a)return null;let l=t.split(`
`),r=l.findIndex(o=>o.includes(`${We}${s}`));if(r===-1)return null;let d=l.slice();return l[r+1]===""?d.splice(r+2,0,a.trim(),""):d.splice(r+1,0,"",a.trim(),""),d.join(`
`)}function de(t,s,a=null){return!t||!s||!t.includes(`plugin://${s}`)?!1:a?t.includes(`att=${a}`):!0}function he(t,s,a){if(!t||!s||!a)return null;let l=t.split(`
...
`),d=`plugin://${s}`,o=r.findIndex(k=>k.includes(d)&&k.includes(`att=${a}`));if(o===-1)return null;let h=r[o],b=h.match(/data="(plugin:\/\/[^"]*)"/);if(!b)return null;let m=b[1],T=m.indexOf("?"),S=T===-1?"":m.slice(T+1),C={...oe(S),attachmentUUID:a,...l},u=Ze(C),f=u?`plugin://${s}?${u}`:`plugin://${s}`,v=r.slice(),w=h.replace(b[0],`data="${f}"`),y=Ke(C.collapsed),E=w.match(/data-aspect-ratio="[^"]*"/);return w=E?w.replace(E[0],`data-aspect-ratio="${y}"`):w.replace(/\s*\/>\s*$/,` data-aspect-ratio="${y}" />`),v[o]=w,v.join(`
`)}function tt(t,s,a,l){return Ee(t,s,a,{collapsed:!!l})}async function nt(t,s,a){let l=await le(t,s);if(!l){let h=await t.getNoteAttachments({uuid:s});return(!(Array.isArray(h)&&h.length>0)||!h.some(m=>m&&m.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.

Attach a PDF with the paperclip button in the note toolbar, then run this again.`),null}let r=await t.getNoteContent({uuid:s});if(de(r,a,l.uuid))return await t.alert(`"${l.name}" is already open in this note - scroll to the viewer.`),l.uuid;let d=ce(a,{attachmentUUID:l.uuid,attachmentName:l.name}),o=et(r,l.uuid,d);return o!==null?(await t.replaceNoteContent({uuid:s},o),l.uuid):(await t.insertNoteContent({uuid:s},`
${d}
`,{atEnd:!0}),l.uuid)}var on="Raw markdown";function an(t){let s=(String(t||"").match(/`+/g)||[]).reduce((a,l)=>Math.max(a,l.length),0);return"`".repeat(Math.max(3,s+1))}async function ot(t,s){let a=await t.getNoteContent({uuid:s});if(typeof a!="string"||a==="")return await t.alert("That note came back empty - nothing to dump."),null;let l=await t.getNoteAttachments({uuid:s}),r=(Array.isArray(l)?l:[]).map(h=>`- ${h&&h.name} | ${h&&h.type} | ${h&&h.uuid}`).join(`
`),d=an(a),o=await t.createNote("PDF Annotator debug - note markdown");return await t.insertNoteContent({uuid:o},`# Attachments
...
Open it and look for how the PDF attachment is referenced.`),o}async function at(t,s,a){if(!s)return"";let l=await le(t,s);if(!l){let d=await t.getNoteAttachments({uuid:s});return(!(Array.isArray(d)&&d.length>0)||!d.some(h=>h&&h.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.

Attach a PDF with the paperclip button in the note toolbar, then type {PDF Annotator} again where you want the viewer.`),""}let r=await t.getNoteContent({uuid:s});return de(r,a,l.uuid)?(await t.alert(`"${l.name}" already has a viewer in this note.

Detach that one first if you want to move it here.`),""):`
${ce(a,{attachmentUUID:l.uuid,attachmentName:l.name})}
`}async function rn(t,s,a,l){let r={uuid:s},d=he(a,t.context.pluginUUID,l);if(d!==null)try{await t.replaceNoteContent(r,d)}catch{}try{await t.replaceNoteContent(r,a)}catch{await t.replaceNoteContent(r,a)}}async function rt(t,s){let{noteUUID:a,attachmentUUID:l,page:r,highlightId:d}=oe(s);if(!a){await t.alert("This link doesn't say which note the PDF lives on - it was likely exported by an older version of this plugin. Re-export the highlight to get a working link.");return}try{let o=await t.getNoteContent({uuid:a}),h=Ee(o,t.context.pluginUUID,l,{page:r,highlightId:d,collapsed:!1});h!==null&&(t.context&&t.context.noteUUID===a?await rn(t,a,h,l):await t.replaceNoteContent({uuid:a},h))}catch{}await t.navigate(`https://www.amplenote.com/notes/${a}`)}function pe(t){if(!t)return null;let s=String(t).trim().toLowerCase();return ie.find(a=>a.id===s||a.hex.toLowerCase()===s)||null}function it(){return pe(se)}function sn(){return"hl-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2,8)}function ue({page:t,color:s,rects:a,quoteText:l,note:r=null,id:d=null}){if(!Number.isInteger(t)||t<1)throw new Error(`createHighlight: page must be a positive integer, got ${t}`);if(!Array.isArray(a)||a.length===0)throw new Error("createHighlight: rects must be a non-empty array");for(let h of a)if(![h.x,h.y,h.width,h.height].every(Number.isFinite))throw new Error(`createHighlight: malformed rect ${JSON.stringify(h)}`);let o=pe(s)||it();return{id:d||sn(),page:t,color:o.id,rects:a.map(h=>({x:h.x,y:h.y,width:h.width,height:h.height})),quoteText:String(l||""),note:r?String(r):null}}function st(t,s){let a=s==null?null:String(s).trim();return{...t,note:a||null}}function lt(t,s){let a=pe(s);if(!a)throw new Error(`withColor: unknown color "${s}"`);return{...t,color:a.id}}function ct(t,s){return(t||[]).filter(a=>a.id!==s)}function Ne(t,s,a){let l=!1,r=(t||[]).map(d=>d.id!==s?d:(l=!0,a(d)));return l?r:t}var ln="json",dt="*Managed automatically by the PDF Annotator plugin - safe to ignore, don't edit.*";function ht(t){let s=JSON.stringify(t,null,0).replace(/`/g,"\\u0060");return`${dt}
\`\`\`${ln}
${s}
\`\`\``}function Se(t){if(!t)return null;let s=t.match(/```(?:json)?\s*\n?([\s\S]*?)\n?```/),a=!s&&t.match(/<!--\s*PDFA-DATA\s*\n?([\s\S]*?)-->/),l=(s?s[1]:a?a[1]:t).trim();if(!l)return null;try{return JSON.parse(l)}catch{return null}}function cn(t){if(!Array.isArray(t))return[];let s=[];for(let a of t)try{s.push(ue(a))}catch{}return s}async function fe(t,s,a){let l=await t.getNoteContent({uuid:s}),r=Ae(l,G),d=Se(r);return!d||typeof d!="object"?[]:cn(d[a])}async function pt(t,s,a,l){let r={uuid:s},d=await t.getNoteContent(r),o=Ae(d,G),b={...Se(o)||{},[a]:l},m=ht(b);o===null&&await t.insertNoteContent(r,`

# ${G}

`,{atEnd:!0});let T=hn(d,m);if(T!==null){await t.replaceNoteContent(r,T);return}await t.replaceNoteContent(r,m,{section:{heading:{text:G,level:1}}})}async function ut(t,s,a){let l={uuid:s},r=await t.getNoteContent(l),d=Ae(r,G);if(d===null)return;let o=Se(d)||{};if(!(a in o))return;let h={...o};delete h[a],await t.replaceNoteContent(l,ht(h),{section:{heading:{text:G,level:1}}})}function Te(t,s){let a=/^#\s+(.*)$/,l=t.findIndex(d=>{let o=d.match(a);return o&&o[1].trim()===s});if(l===-1)return null;let r=t.length;for(let d=l+1;d<t.length;d++)if(/^#\s+/.test(t[d])){r=d;break}return{start:l,end:r}}function Ae(t,s){if(!t)return null;let a=t.split(`
`),l=Te(a,s);return l?a.slice(l.start+1,l.end).join(`
`).trim():null}function dn(t){if(!t)return"";let s=t,a=s.match(/```(?:json)?\s*\n?[\s\S]*?\n?```/);return a&&(s=s.replace(a[0],"")),s=s.replace(/<!--\s*PDFA-DATA[\s\S]*?-->/,""),s=s.replace(dt,""),s.trim()}function ft(t,s){let a=(t||"").split(`
...
${d}`}function hn(t,s){let a=(t||"").split(`
`),l=Te(a,G);if(!l)return null;let r=dn(a.slice(l.start+1,l.end).join(`
...
`+o:""}`}function z(t,s){return s.noteUUID||t.context.noteUUID}async function gt(t,s,a){try{let l=await t.getNoteAttachments({uuid:s}),r=Array.isArray(l)&&l.find(d=>d&&d.uuid===a);return r?r.name:""}catch{return""}}async function ge(t,s,a,l){let r=await fe(t,s,a),d=l(r);return d!==r&&await pt(t,s,a,d),{highlights:d}}function mt(t){if(t&&typeof t=="object")return t;if(typeof t!="string")return{};let s=t.trim();if(!s.startsWith("{"))return{action:s};try{return JSON.parse(s)}catch{return{action:s}}}async function vt(t,s){return JSON.stringify(await pn(t,mt(s)))}async function pn(t,s){let a=mt(s);switch(a.action){case"getPdfUrl":{let l=a.attachmentUUID;if(!l)return{error:"No attachment specified for this viewer."};try{return{url:await Qe(t,l),name:await gt(t,z(t,a),l)}}catch(r){return{error:`Could not load the PDF: ${r.message}`}}}case"loadHighlights":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return{highlights:await fe(t,z(t,a),a.attachmentUUID)}}catch(l){return{error:`Could not load highlights: ${l.message}`}}}case"addHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{let l=ue(a.highlight||{});return await ge(t,z(t,a),a.attachmentUUID,r=>r.concat([l]))}catch(l){return{error:`Could not save the highlight: ${l.message}`}}}case"recolorHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ge(t,z(t,a),a.attachmentUUID,l=>Ne(l,a.id,r=>lt(r,a.color)))}catch(l){return{error:`Could not change the highlight color: ${l.message}`}}}case"setHighlightNote":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ge(t,z(t,a),a.attachmentUUID,l=>Ne(l,a.id,r=>st(r,a.note)))}catch(l){return{error:`Could not save the note: ${l.message}`}}}case"removeHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ge(t,z(t,a),a.attachmentUUID,l=>ct(l,a.id))}catch(l){return{error:`Could not remove the highlight: ${l.message}`}}}case"sendToNote":{if(!a.content)return{error:"Nothing to send."};try{let l={uuid:z(t,a)},r=await t.getNoteContent(l),d=ft(r,a.content);return d===null?await t.insertNoteContent(l,`
`+a.content+`
`,{atEnd:!0}):await t.replaceNoteContent(l,d),{ok:!0}}catch(l){return{error:`Could not add this to the note: ${l.message}`}}}case"removeViewer":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!a.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=z(t,a),r=await t.getNoteContent({uuid:l}),d=he(r,a.pluginUUID,a.attachmentUUID);return d===null?{error:"Could not find this viewer's block in the note - it may already be removed."}:(await t.replaceNoteContent({uuid:l},d),await ut(t,l,a.attachmentUUID),{ok:!0})}catch(l){return{error:`Could not remove this viewer: ${l.message}`}}}case"getViewerSummary":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};let l=z(t,a),r=await gt(t,l,a.attachmentUUID);try{let d=await fe(t,l,a.attachmentUUID);return{name:r,count:d.length}}catch{return{name:r,count:0}}}case"setCollapsed":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!a.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=z(t,a),r=await t.getNoteContent({uuid:l}),d=tt(r,a.pluginUUID,a.attachmentUUID,a.collapsed);return d===null?{ok:!1}:(await t.replaceNoteContent({uuid:l},d),{ok:!0})}catch(l){return{error:`Could not resize this viewer: ${l.message}`}}}case"exportAll":{if(!a.noteName)return{error:"Missing destination note name."};try{let l=await t.findNote({name:a.noteName}),r=l?l.uuid:await t.createNote(a.noteName);return await t.replaceNoteContent({uuid:r},a.content||""),{ok:!0,noteUUID:r}}catch(l){return{error:`Could not export highlights: ${l.message}`}}}case"ping":return{ok:!0};default:return{error:`Unknown embed action: ${String(a.action)}`}}}function ke(){function t(u,f){return{x:u.left-f.left,y:u.top-f.top,width:u.width,height:u.height}}function s(u,f){return{x:Math.min(u[0],f[0]),y:Math.min(u[1],f[1]),width:Math.abs(f[0]-u[0]),height:Math.abs(f[1]-u[1])}}function a(u,f){var v=Math.pow(10,f===void 0?2:f),w=function(y){return Math.round(y*v)/v};return{x:w(u.x),y:w(u.y),width:w(u.width),height:w(u.height)}}function l(u){return u.width>.01&&u.height>.01}function r(u,f,v){for(var w=String(u??""),y=Math.max(0,f===void 0?0:f),E=Math.min(w.length,v===void 0?w.length:v),k=function(O){return O===""||/\s/.test(O)},D=[],P=y;P<E;){for(;P<E&&k(w.charAt(P));)P++;if(P>=E)break;for(var $=P;P<E&&!k(w.charAt(P));)P++;D.push({start:$,end:P})}return D}function d(u){for(var f=1/0,v=1/0,w=-1/0,y=-1/0,E=0;E<(u?u.length:0);E++){var k=u[E];l(k)&&(f=Math.min(f,k.left),v=Math.min(v,k.top),w=Math.max(w,k.left+k.width),y=Math.max(y,k.top+k.height))}return isFinite(f)?{left:f,top:v,width:w-f,height:y-v}:null}function o(u,f,v){for(var w=[],y=0;y<u.length;y++){var E=t(u[y],f);if(l(E)){var k=v(E.x,E.y),D=v(E.x+E.width,E.y+E.height),P=a(s(k,D));l(P)&&w.push(P)}}return w}function h(u,f){var v=f(u.x,u.y),w=f(u.x+u.width,u.y+u.height);return s(v,w)}function b(u,f,v){var w=f.right-f.left,y=f.bottom-f.top;if(w<=0||y<=0)return null;var E=u.x2-u.x1,k=u.y2-u.y1,D=u.x1+(v.left-f.left)/w*E,P=u.x2-(f.right-v.right)/w*E,$=u.y1+(v.bottom-f.bottom)/y*k,O=u.y2-(f.top-v.top)/y*k;return{x:D,y:$,width:P-D,height:O-$}}function m(u,f){var v=Math.min(u.y+u.height,f.y+f.height)-Math.max(u.y,f.y);return v>.5*Math.min(u.height,f.height)}function T(u,f){var v=f===void 0?.6:f;if(!u||u.length<2)return(u||[]).slice();for(var w=u.slice().sort(function(j,X){return X.y-j.y||j.x-X.x}),y=[],E=0;E<w.length;E++){for(var k=!1,D=0;D<y.length;D++)if(m(y[D][0],w[E])){y[D].push(w[E]),k=!0;break}k||y.push([w[E]])}for(var P=[],$=0;$<y.length;$++){for(var O=y[$].slice().sort(function(j,X){return j.x-X.x}),H=null,J=0;J<O.length;J++){var F=O[J];if(H===null){H={x:F.x,y:F.y,width:F.width,height:F.height};continue}var ae=F.x-(H.x+H.width);if(ae<=v*Math.max(H.height,F.height)){var ee=Math.max(H.x+H.width,F.x+F.width),me=Math.max(H.y+H.height,F.y+F.height);H.x=Math.min(H.x,F.x),H.y=Math.min(H.y,F.y),H.width=ee-H.x,H.height=me-H.y}else P.push(H),H={x:F.x,y:F.y,width:F.width,height:F.height}}H!==null&&P.push(H)}return P.map(function(j){return a(j)})}function S(u,f,v,w){var y=w===void 0?0:w;return f>=u.x-y&&f<=u.x+u.width+y&&v>=u.y-y&&v<=u.y+u.height+y}function N(u,f,v,w,y){for(var E=u||[],k=E.length-1;k>=0;k--){var D=E[k];if(!(!D||D.page!==f||!D.rects)){for(var P=0;P<D.rects.length;P++)if(S(D.rects[P],v,w,y===void 0?1:y))return D}}return null}function C(u){return String(u??"").replace(/\s+/g," ").trim()}return{clientRectToLocal:t,rectFromCorners:s,roundRect:a,isVisibleRect:l,textTokenRanges:r,unionClientRects:d,clientRectsToPdfRects:o,pdfRectToViewportRect:h,itemRelativeRect:b,mergeLineRects:T,rectContainsPoint:S,hitTestHighlights:N,normalizeQuoteText:C}}var B=ke(),Vn=B.clientRectToLocal,Wn=B.rectFromCorners,Jn=B.roundRect,Xn=B.isVisibleRect,Yn=B.textTokenRanges,Qn=B.unionClientRects,Kn=B.clientRectsToPdfRects,Zn=B.pdfRectToViewportRect,eo=B.itemRelativeRect,to=B.mergeLineRects,no=B.rectContainsPoint,oo=B.hitTestHighlights,ao=B.normalizeQuoteText;function Ie(){var t=[.957,.871,.424];function s(d,o,h,b,m){var T=o.context.register(o.context.obj({Type:d.PDFName.of("ExtGState"),BM:d.PDFName.of("Multiply"),ca:d.PDFNumber.of(.4)})),S=[d.pushGraphicsState(),d.setGraphicsState("GS0")];S.push(d.setFillingColor(d.rgb(b[0],b[1],b[2])));for(var N=0;N<h.length;N++){var C=h[N];S.push(d.moveTo(C.x,C.y)),S.push(d.lineTo(C.x,C.y+C.height)),S.push(d.lineTo(C.x+C.width,C.y+C.height)),S.push(d.lineTo(C.x+C.width,C.y)),S.push(d.closePath())}S.push(d.fill()),S.push(d.popGraphicsState());var u=o.context.formXObject(S,{BBox:m,Resources:{ExtGState:{GS0:T}}});return o.context.register(u)}function a(d,o,h,b){for(var m=h.rects,T=[],S=m[0].x,N=m[0].y,C=m[0].x+m[0].width,u=m[0].y+m[0].height,f=0;f<m.length;f++){var v=m[f],w=v.x,y=v.x+v.width,E=v.y,k=v.y+v.height;T.push(w,k,y,k,w,E,y,E),S=Math.min(S,w),N=Math.min(N,E),C=Math.max(C,y),u=Math.max(u,k)}var D=o.context.obj({Type:d.PDFName.of("Annot"),Subtype:d.PDFName.of("Highlight"),Rect:o.context.obj([S,N,C,u]),QuadPoints:o.context.obj(T),C:o.context.obj(b),F:d.PDFNumber.of(4),T:d.PDFString.of("PDF Annotator"),M:d.PDFString.of(new Date().toISOString()),CA:d.PDFNumber.of(.4)});h.note&&D.set(d.PDFName.of("Contents"),d.PDFString.of(h.note));var P=s(d,o,m,b,[S,N,C,u]);D.set(d.PDFName.of("AP"),o.context.obj({N:P}));var $=o.context.register(D),O=[$];if(h.note){var H=o.context.register(o.context.obj({Type:d.PDFName.of("Annot"),Subtype:d.PDFName.of("Popup"),Rect:o.context.obj([C+8,N-60,C+208,N+12]),Parent:$,Open:!1}));D.set(d.PDFName.of("Popup"),H),O.push(H)}return O}function l(d,o,h){var b=o.node.get(d.PDFName.of("Annots"));if(b instanceof d.PDFArray)for(var m=0;m<h.length;m++)b.push(h[m]);else o.node.set(d.PDFName.of("Annots"),o.doc.context.obj(h))}async function r(d,o,h,b){for(var m=await d.PDFDocument.load(o),T=m.getPages(),S=h||[],N=0;N<S.length;N++){var C=S[N];if(!(!C||!C.rects||!C.rects.length)){var u=T[C.page-1];if(u){var f=b&&b[C.color]||t,v=a(d,m,C,f);l(d,u,v)}}}return m.save()}return{writeHighlightsIntoPdf:r,buildHighlightAnnotation:a,appendAnnotationRefs:l}}var Pe=Ie(),io=Pe.writeHighlightsIntoPdf,so=Pe.buildHighlightAnnotation,lo=Pe.appendAnnotationRefs;function De(){function t(o){return String(o??"").replace(/\]/g,"\\]")}function s(o,h,b,m,T){var S=new URLSearchParams;h&&S.set("att",h),Number.isFinite(b)&&b>=1&&S.set("page",String(Math.floor(b))),m&&S.set("hl",m),T&&S.set("note",T);var N=S.toString();return"plugin://"+o+(N?"?"+N:"")}function a(o,h){return String(o??"").split(/\r?\n/).map(function(b){return(h+" "+b).replace(/[ \t]+$/,"")})}function l(o,h,b,m,T,S){var N=s(h,b,m.page,m.id,S),C=t(o||"PDF"),u='==\u25CF<!-- {"cycleColor":"'+T+'"} -->==',f=u+" ["+C+"]("+N+")",v=[f].concat(a(m.quoteText,"> >"));return m.note&&(v.push(">"),v=v.concat(a(m.note,">"))),v.join(`
...
`)}return{buildDeepLink:s,buildHighlightBlock:l,buildExportAllContent:d}}var Ue=De(),ho=Ue.buildDeepLink,po=Ue.buildHighlightBlock,uo=Ue.buildExportAllContent;function wt(){var t=window.__PDFA_CONFIG||{},s=window.__PDFA_GEOM||{},a=window.__PDFA_ANNOTATIONS||{},l=window.__PDFA_EXPORT||{},r={root:document.getElementById("pdfa-root"),pages:document.getElementById("pdfa-pages"),status:document.getElementById("pdfa-status"),pageLabel:document.getElementById("pdfa-page-label"),zoomLabel:document.getElementById("pdfa-zoom-label"),colors:document.getElementById("pdfa-colors"),hint:document.getElementById("pdfa-hint"),popover:document.getElementById("pdfa-popover"),panel:document.getElementById("pdfa-panel"),listToggle:document.getElementById("pdfa-list-toggle"),count:document.getElementById("pdfa-count"),more:document.getElementById("pdfa-more"),open:document.getElementById("pdfa-open"),scrollUp:document.getElementById("pdfa-scroll-up"),scrollDown:document.getElementById("pdfa-scroll-down"),collapsedCount:document.getElementById("pdfa-collapsed-count"),collapsedName:document.querySelector(".pdfa-collapsed-name"),name:document.querySelector(".pdfa-name")};function d(e){e&&(o.attachmentName=e,r.name&&(r.name.textContent=e),r.collapsedName&&(r.collapsedName.textContent=e))}var o={doc:null,scale:1.25,pageCount:0,current:1,rendering:!1,textSpans:0,viewports:{},rendered:{},renderingPage:{},highlights:[],pdfBytes:null,attachmentName:t.attachmentName||"",activeColorId:t.defaultColorId||((t.colors||[{}])[0]||{}).id,pendingSelection:null,lastCapturedText:"",noteEditing:null};function h(e,n){r.status.textContent=e||"",r.status.style.display=e?"block":"none",r.status.className=n?"pdfa-status pdfa-error":"pdfa-status"}function b(e){var n=Object.assign({noteUUID:t.noteUUID},e);return new Promise(function(i,c){try{if(typeof window.callAmplenotePlugin!="function")throw new Error("Plugin bridge unavailable (callAmplenotePlugin missing)");i(window.callAmplenotePlugin(JSON.stringify(n)))}catch(p){c(p)}}).then(function(i){if(i&&typeof i=="object")return i;if(typeof i!="string")throw new Error("Empty reply from the plugin");try{return JSON.parse(i)}catch{throw new Error("Unreadable reply from the plugin: "+String(i).slice(0,120))}})}function m(){return t.colors||[]}function T(e){for(var n=m(),i=0;i<n.length;i++)if(n[i].id===e)return n[i].hex;return n.length?n[0].hex:"#F4DE6C"}function S(e){for(var n=0;n<o.highlights.length;n++)if(o.highlights[n].id===e)return o.highlights[n];return null}function N(e,n,i){var c=document.createElement("button");return c.className="pdfa-btn"+(n?" "+n:""),c.textContent=e,c.onclick=function(p){p.stopPropagation(),i()},c}function C(e,n,i,c){var p=document.createElement("button");return p.className="pdfa-color",p.dataset.color=e.id,p.style.background=e.hex,p.title=c+" "+e.label,p.setAttribute("aria-label",c+" "+e.label),p.setAttribute("aria-pressed",String(!!n)),p.onclick=function(x){x.stopPropagation(),i(e.id)},p}function u(){for(var e=m(),n=0;n<e.length;n++)r.colors.appendChild(C(e[n],e[n].id===o.activeColorId,function(i){o.activeColorId=i,f(),o.pendingSelection&&Re(o.pendingSelection,i)},"Highlight"))}function f(){for(var e=r.colors.querySelectorAll(".pdfa-color"),n=0;n<e.length;n++)e[n].setAttribute("aria-pressed",String(e[n].dataset.color===o.activeColorId))}function v(){for(var e=[],n=1;n<=o.pageCount;n++)(function(i){e.push(o.doc.getPage(i).then(function(c){o.viewports[i]=c.getViewport({scale:o.scale})}))})(n);return Promise.all(e)}function w(e){var n=o.viewports[e],i=document.createElement("div");return i.className="pdfa-page",i.dataset.page=String(e),i.style.width=n.width+"px",i.style.height=n.height+"px",i}function y(e,n){if(o.rendered[n]||o.renderingPage[n])return Promise.resolve();o.renderingPage[n]=!0;var i=o.viewports[n],c=document.createElement("canvas"),p=window.devicePixelRatio||1;c.width=Math.floor(i.width*p),c.height=Math.floor(i.height*p),c.style.width=i.width+"px",c.style.height=i.height+"px",e.appendChild(c);var x=document.createElement("div");x.className="pdfa-highlights",e.appendChild(x);var g=document.createElement("div");g.className="textLayer",g.style.width=i.width+"px",g.style.height=i.height+"px",g.style.setProperty("--scale-factor",String(o.scale)),e.appendChild(g);var A=c.getContext("2d");A.scale(p,p);var R=null;return o.doc.getPage(n).then(function(U){return R=U,U.render({canvasContext:A,viewport:i}).promise}).then(function(){return R.getTextContent()}).then(function(U){var I=[];return window.pdfjsLib.renderTextLayer({textContent:U,container:g,viewport:i,textDivs:I}).promise.then(function(){o.textSpans+=I.length;for(var L=0;L<I.length;L++)I[L].__pdfaItem=U.items[L];o.rendered[n]=!0,o.renderingPage[n]=!1,$(n),k()})}).catch(function(U){o.renderingPage[n]=!1,h("Failed to render page "+n+": "+(U.message||U),!0)})}function E(){var e=V();if(!e||!o.doc)return Promise.resolve();for(var n=e.getBoundingClientRect(),i=e.clientHeight,c=r.pages.querySelectorAll(".pdfa-page"),p=[],x=0;x<c.length;x++){var g=c[x],A=Number(g.dataset.page);if(!(o.rendered[A]||o.renderingPage[A])){var R=g.getBoundingClientRect(),U=R.top-n.top,I=R.bottom-n.top;I<-i||U>e.clientHeight+i||p.push(y(g,A))}}return Promise.all(p)}function k(){var e=0;for(var n in o.rendered)o.rendered[n]&&e++;if(e){var i=o.textSpans===0;h(i?"No selectable text found - this PDF may be a scan.":"",i)}}function D(){if(o.rendering)return Promise.resolve();o.rendering=!0,M(!0),h("Rendering...");var e=V(),n=e?e.scrollHeight-e.clientHeight:0,i=n>0?e.scrollTop/n:0;return r.pages.innerHTML="",o.viewports={},o.rendered={},o.renderingPage={},o.textSpans=0,v().then(function(){for(var c=1;c<=o.pageCount;c++)r.pages.appendChild(w(c));if(e){var p=e.scrollHeight-e.clientHeight;e.scrollTop=i*(p>0?p:0)}o.rendering=!1,te(),be(),E()}).catch(function(c){o.rendering=!1,h("Failed to render: "+(c.message||c),!0)})}function P(e){return function(n,i){return e.convertToViewportPoint(n,i)}}function $(e){for(var n=e?'.pdfa-page[data-page="'+e+'"]':".pdfa-page",i=r.pages.querySelectorAll(n),c=0;c<i.length;c++){var p=i[c],x=Number(p.dataset.page),g=p.querySelector(".pdfa-highlights"),A=o.viewports[x];if(!(!g||!A)){g.innerHTML="";for(var R=P(A),U=0;U<o.highlights.length;U++){var I=o.highlights[U];if(!(!I||I.page!==x||!I.rects||!I.rects.length)){var L=document.createElement("div");L.className="pdfa-hl-group",L.dataset.id=I.id||"";for(var W=0;W<I.rects.length;W++){var K=s.pdfRectToViewportRect(I.rects[W],R),_=document.createElement("div");_.className="pdfa-hl",_.style.left=K.x+"px",_.style.top=K.y+"px",_.style.width=K.width+"px",_.style.height=K.height+"px",_.style.background=T(I.color),L.appendChild(_)}g.appendChild(L)}}}}}function O(){$(),J(),r.count.textContent=String(o.highlights.length)}function H(){return o.highlights.slice().sort(function(e,n){return e.page!==n.page?e.page-n.page:(n.rects[0]?n.rects[0].y:0)-(e.rects[0]?e.rects[0].y:0)})}function J(){r.panel.innerHTML="";var e=document.createElement("div");e.className="pdfa-panel-title";var n=document.createElement("span");n.textContent="Highlights",e.appendChild(n),e.appendChild(N("Close","",function(){ae(!1)})),r.panel.appendChild(e);var i=H();if(!i.length){var c=document.createElement("div");c.className="pdfa-panel-empty",c.textContent="No highlights yet. Select some text in the PDF and pick a color.",r.panel.appendChild(c);return}for(var p=0;p<i.length;p++)r.panel.appendChild(F(i[p]))}function F(e){var n=document.createElement("div");n.className="pdfa-hl-row",n.dataset.id=e.id||"",n.title="Jump to this highlight";var i=document.createElement("span");i.className="pdfa-chip",i.style.background=T(e.color),n.appendChild(i);var c=document.createElement("div"),p=document.createElement("div");p.className="pdfa-hl-page",p.textContent="Page "+e.page,c.appendChild(p);var x=document.createElement("div");if(x.className="pdfa-hl-quote",x.textContent=e.quoteText.length>160?e.quoteText.slice(0,160)+"...":e.quoteText,c.appendChild(x),e.note){var g=document.createElement("div");g.className="pdfa-hl-note",g.textContent=e.note,c.appendChild(g)}return n.appendChild(c),n.onclick=function(){Fe(e)},n}function ae(e){var n=e===void 0?!r.panel.classList.contains("pdfa-open"):e;r.panel.classList.toggle("pdfa-open",n),r.listToggle.setAttribute("aria-pressed",String(n)),n&&J()}function ee(e){for(var n=e&&e.nodeType===1?e:e&&e.parentElement;n;){if(n.classList&&n.classList.contains("textLayer"))return n;n=n.parentElement}return null}function me(e,n){for(var i=[],c=[],p=null,x=document.createTreeWalker(n,NodeFilter.SHOW_TEXT,null),g;g=x.nextNode();)if(e.intersectsNode(g)){var A=g.nodeValue||"",R=g===e.startContainer?e.startOffset:0,U=g===e.endContainer?e.endOffset:A.length,I=g.parentElement,L=I&&I.__pdfaItem;if(L)for(var W={x1:L.transform[4],y1:L.transform[5],x2:L.transform[4]+L.width,y2:L.transform[5]+L.height},K=I.getBoundingClientRect(),_=s.textTokenRanges(A,R,U),Z=0;Z<_.length;Z++){var ye=document.createRange();ye.setStart(g,_[Z].start),ye.setEnd(g,_[Z].end);var q=s.unionClientRects(ye.getClientRects());if(q){var Ge={left:q.left,top:q.top,width:q.width,height:q.height,right:q.left+q.width,bottom:q.top+q.height},Ve=s.itemRelativeRect(W,K,Ge);Ve&&(i.push(Ve),c.push(A.slice(_[Z].start,_[Z].end)),p=Ge)}}}return{rects:i,text:c.join(" "),lastCssRect:p}}function j(e){if(o.pendingSelection=e,o.lastCapturedText=e&&e.rawText||"",!e){r.hint.textContent="",r.hint.style.display="none";return}r.hint.textContent=e.spilled?"Pick a color (page "+e.page+" only)":"Pick a color",r.hint.style.display="inline"}function X(e){if(!o.noteEditing){var n=window.getSelection();if(!n||n.isCollapsed||n.rangeCount===0){j(null),M();return}var i=n.getRangeAt(0),c=ee(i.startContainer);if(!c)return j(null);var p=c.parentElement;if(!p||!p.dataset||!p.dataset.page)return j(null);var x=Number(p.dataset.page);if(!o.rendered[x])return j(null);var g=ee(i.endContainer)!==c,A=me(i,c),R=s.mergeLineRects(A.rects);if(!R.length)return j(null);var U=A.lastCssRect||p.getBoundingClientRect(),I=e&&e.clientX?e.clientX:U.left+U.width/2,L=e&&e.clientY?e.clientY:U.top+U.height,W={page:x,rects:R,quoteText:s.normalizeQuoteText(A.text),spilled:g,anchorX:I,anchorY:L,rawText:String(n)};j(W),St(W)}}var yt=300,Y=null;function Ct(){o.noteEditing||(Y&&clearTimeout(Y),Y=setTimeout(He,yt))}function He(){if(Y=null,!o.noteEditing){var e=window.getSelection();!e||e.isCollapsed||e.rangeCount===0||ee(e.getRangeAt(0).startContainer)&&String(e)!==o.lastCapturedText&&X(null)}}function re(e,n){var i=o.highlights;return o.highlights=e,O(),b(n).then(function(c){if(!c||c.error)throw new Error(c&&c.error||"The plugin did not confirm the change.");return o.highlights=c.highlights||e,O(),h(""),!0}).catch(function(c){return o.highlights=i,O(),h(c.message||String(c),!0),!1})}function Re(e,n){var i={id:null,page:e.page,color:n,rects:e.rects,quoteText:e.quoteText,note:null},c=e.anchorX,p=e.anchorY;j(null),M(!0);var x=window.getSelection();x&&x.removeAllRanges&&x.removeAllRanges(),re(o.highlights.concat([i]),{action:"addHighlight",attachmentUUID:t.attachmentUUID,highlight:i}).then(function(g){if(g){var A=o.highlights[o.highlights.length-1];A&&A.id&&we(A,c,p,!0)}})}function Et(e,n){M(!0),re(o.highlights.map(function(i){return i.id===e?Object.assign({},i,{color:n}):i}),{action:"recolorHighlight",attachmentUUID:t.attachmentUUID,id:e,color:n})}function Nt(e){M(!0),re(o.highlights.filter(function(n){return n.id!==e}),{action:"removeHighlight",attachmentUUID:t.attachmentUUID,id:e})}function ve(e,n){var i=String(n??"").trim();o.noteEditing=null,M(!0),re(o.highlights.map(function(c){return c.id===e?Object.assign({},c,{note:i||null}):c}),{action:"setHighlightNote",attachmentUUID:t.attachmentUUID,id:e,note:i})}function Q(e,n,i,c){r.popover.innerHTML="",r.popover.classList.toggle("pdfa-editing",c==="editing"),r.popover.classList.toggle("pdfa-exporting",c==="exporting"),r.popover.classList.toggle("pdfa-menu",c==="menu");for(var p=0;p<e.length;p++)r.popover.appendChild(e[p]);r.popover.classList.add("pdfa-open");var x=r.popover.offsetWidth,g=r.popover.offsetHeight,A=Math.max(4,Math.min(n-x/2,window.innerWidth-x-4)),R=i+12;R+g>window.innerHeight-4&&(R=Math.max(4,i-g-12)),r.popover.style.left=A+"px",r.popover.style.top=R+"px"}function M(e){o.noteEditing&&!e||(o.noteEditing=null,r.popover.classList.remove("pdfa-open","pdfa-editing","pdfa-exporting","pdfa-menu"),r.popover.innerHTML="")}function St(e){for(var n=m(),i=[],c=0;c<n.length;c++)i.push(C(n[c],n[c].id===o.activeColorId,function(p){o.activeColorId=p,f(),Re(e,p)},"Highlight"));Q(i,e.anchorX,e.anchorY)}function we(e,n,i,c){for(var p=m(),x=[],g=0;g<p.length;g++)x.push(C(p[g],p[g].id===e.color,function(R){Et(e.id,R)},"Change to"));var A=!!e.note;x.push(N(A?"Edit note":"Add note",c&&!A?"pdfa-btn-primary":"",function(){At(e,n,i)})),x.push(N("Copy","",function(){Ot(e)})),x.push(N("Send to note","",function(){$t(e)})),x.push(N("Remove","pdfa-remove",function(){Nt(e.id)})),Q(x,n,i)}function Tt(e,n){for(var i=m(),c={},p=0;p<i.length;p++)c[i[p].id]=!0;var x=document.createElement("div");x.className="pdfa-export-hint",x.textContent="Export highlights to a note";var g=document.createElement("div");g.className="pdfa-export-colors";for(var A=0;A<i.length;A++)(function(U){var I=C(U,!0,function(L){c[L]=!c[L],I.setAttribute("aria-pressed",String(c[L]))},"Toggle");g.appendChild(I)})(i[A]);var R=document.createElement("div");R.className="pdfa-note-actions",R.appendChild(N("Create / update note","pdfa-btn-primary",function(){for(var U=[],I=0;I<i.length;I++)c[i[I].id]&&U.push(i[I].id);jt(U.length===i.length?null:U)})),Q([x,g,R],e,n,"exporting")}function At(e,n,i){o.noteEditing=e.id;var c=document.createElement("textarea");c.className="pdfa-note-input",c.rows=3,c.value=e.note||"",c.placeholder="Note for this highlight";var p=document.createElement("div");p.className="pdfa-note-actions",e.note&&p.appendChild(N("Delete note","",function(){ve(e.id,"")}));var x=document.createElement("span");x.className="pdfa-spacer",p.appendChild(x),p.appendChild(N("Cancel","",function(){Le(e,n,i)})),p.appendChild(N("Save","pdfa-btn-primary",function(){ve(e.id,c.value)})),c.onkeydown=function(g){g.key==="Enter"&&(g.ctrlKey||g.metaKey)?(g.preventDefault(),g.stopPropagation(),ve(e.id,c.value)):g.key==="Escape"&&(g.preventDefault(),g.stopPropagation(),Le(e,n,i))},Q([c,p],n,i,"editing"),c.focus(),c.setSelectionRange(c.value.length,c.value.length)}function Le(e,n,i){o.noteEditing=null;var c=S(e.id)||e;we(c,n,i)}function kt(e){if(!o.noteEditing){var n=window.getSelection();if(!(n&&!n.isCollapsed)){for(var i=e.target,c=null;i&&i!==r.pages;){if(i.classList&&i.classList.contains("pdfa-page")){c=i;break}i=i.parentElement}if(!c)return M();var p=Number(c.dataset.page),x=o.viewports[p];if(!x)return M();var g=c.getBoundingClientRect(),A=x.convertToPdfPoint(e.clientX-g.left,e.clientY-g.top),R=s.hitTestHighlights(o.highlights,p,A[0],A[1],1);R&&R.id?we(R,e.clientX,e.clientY):M()}}}function te(){r.pageLabel.textContent=o.current+" / "+o.pageCount,r.zoomLabel.textContent=Math.round(o.scale*100)+"%"}function V(){return r.root.querySelector(".pdfa-scroll")}function Me(e){var n=r.pages.querySelector('.pdfa-page[data-page="'+e+'"]');n&&y(n,e)}function xe(e){var n=Math.min(Math.max(1,e),o.pageCount),i=r.pages.querySelector('.pdfa-page[data-page="'+n+'"]');Me(n);var c=V();i&&c&&(c.scrollTop+=i.getBoundingClientRect().top-c.getBoundingClientRect().top),E(),o.current=n,te()}function Fe(e){var n=r.pages.querySelector('.pdfa-page[data-page="'+e.page+'"]'),i=o.viewports[e.page];if(!(!n||!i||!e.rects||!e.rects.length)){var c=s.pdfRectToViewportRect(e.rects[0],P(i)),p=V(),x=n.getBoundingClientRect().top+c.y;p.scrollTop+=x-p.getBoundingClientRect().top-p.clientHeight/3,Me(e.page),E(),o.current=e.page,te()}}function It(){try{r.root.setAttribute("tabindex","-1"),r.root.focus(),r.root.scrollIntoView&&r.root.scrollIntoView({block:"nearest"})}catch{}}function Pt(e){if(!(!e||!e.id)){var n=r.pages.querySelector('.pdfa-hl-group[data-id="'+e.id+'"]');n&&(n.classList.add("pdfa-hl-flash"),setTimeout(function(){n.classList.remove("pdfa-hl-flash")},2600))}}function Oe(e){return o.scale=Math.min(Math.max(.4,e),4),D()}function Dt(){return o.doc?o.doc.getPage(1).then(function(e){var n=V();if(n){var i=window.getComputedStyle(n),c=n.clientWidth-(parseFloat(i.paddingLeft)||0)-(parseFloat(i.paddingRight)||0),p=e.getViewport({scale:1}).width;if(!(!(c>0)||!(p>0))){var x=Math.max(.4,c/p);x<o.scale&&(o.scale=x,te())}}}).catch(function(){}):Promise.resolve()}function $e(e){var n=V();n&&(n.scrollTop+=e*Math.max(80,n.clientHeight*.85),be(),E())}function be(){var e=V();if(!(!e||!r.scrollUp)){var n=e.scrollHeight-e.clientHeight;r.scrollUp.disabled=e.scrollTop<=1,r.scrollDown.disabled=e.scrollTop>=n-1}}function Ut(){be(),E(),M();for(var e=r.pages.querySelectorAll(".pdfa-page"),n=o.current,i=1/0,c=0;c<e.length;c++){var p=Math.abs(e[c].getBoundingClientRect().top-60);p<i&&(i=p,n=Number(e[c].dataset.page))}n!==o.current&&(o.current=n,te())}function Ht(){return new Promise(function(e,n){if(window.pdfjsLib)return e(window.pdfjsLib);var i=document.createElement("script");i.src=t.pdfJsSrc,i.onload=function(){window.pdfjsLib?e(window.pdfjsLib):n(new Error("PDF.js loaded but did not register itself."))},i.onerror=function(){n(new Error("Could not load PDF.js from the CDN."))},document.head.appendChild(i)})}function Rt(){return new Promise(function(e,n){if(window.PDFLib)return e(window.PDFLib);var i=document.createElement("script");i.src=t.pdfLibSrc,i.onload=function(){window.PDFLib?e(window.PDFLib):n(new Error("pdf-lib loaded but did not register itself."))},i.onerror=function(){n(new Error("Could not load pdf-lib from the CDN."))},document.head.appendChild(i)})}function Lt(){for(var e={},n=m(),i=0;i<n.length;i++)n[i].rgb&&(e[n[i].id]=n[i].rgb);return e}function Mt(){var e=(o.attachmentName||"annotated").replace(/\.pdf$/i,"");return e+"-annotated.pdf"}function je(){for(var e={},n=m(),i=0;i<n.length;i++)n[i].cycleIndex!==void 0&&(e[n[i].id]=n[i].cycleIndex);return e}function _e(){var e=(o.attachmentName||"PDF").replace(/\.pdf$/i,"");return e+" - Highlights"}function Be(e){return l.buildHighlightBlock(o.attachmentName,t.pluginUUID,t.attachmentUUID,e,je()[e.color],t.noteUUID)}function Ft(e){return navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(e):new Promise(function(n,i){var c=document.createElement("textarea");c.value=e,c.style.position="fixed",c.style.left="-9999px",document.body.appendChild(c),c.focus(),c.select();var p=!1;try{p=document.execCommand("copy")}catch{p=!1}document.body.removeChild(c),p?n():i(new Error("Clipboard access is unavailable here."))})}function Ot(e){M(!0),Ft(Be(e)).then(function(){h("Highlight copied - paste it into any note.")}).catch(function(n){h("Could not copy: "+(n.message||n),!0)})}function $t(e){M(!0),b({action:"sendToNote",content:Be(e)}).then(function(n){if(!n||n.error)throw new Error(n&&n.error||"Could not send this to the note.");h("Sent to the bottom of this note.")}).catch(function(n){h(n.message||String(n),!0)})}function jt(e){M(!0);var n=l.buildExportAllContent(o.attachmentName,t.pluginUUID,t.attachmentUUID,o.highlights,je(),e,t.noteUUID);if(!n){h(e?"No highlights match those colors.":"No highlights to export yet.",!0);return}b({action:"exportAll",noteName:_e(),content:n}).then(function(i){if(!i||i.error)throw new Error(i&&i.error||"Could not export highlights.");h('Exported to "'+_e()+'".')}).catch(function(i){h(i.message||String(i),!0)})}function _t(e,n){var i=document.createElement("div");i.className="pdfa-menu-name",i.textContent=o.attachmentName||"PDF Annotator",i.title=i.textContent;var c=[i];c.push(N("Collapse","",function(){M(!0),Vt()}),N("Download","",function(){M(!0),qt()}),N("Export...","",function(){Tt(e,n)}),N("Remove viewer...","pdfa-remove",function(){Bt(e,n)})),Q(c,e,n,"menu")}function Bt(e,n){var i=document.createElement("div");i.className="pdfa-export-hint",i.textContent="Remove this viewer and all its highlights from this note? This cannot be undone.";var c=document.createElement("div");c.className="pdfa-note-actions",c.appendChild(N("Cancel","",function(){M(!0)}));var p=document.createElement("span");p.className="pdfa-spacer",c.appendChild(p),c.appendChild(N("Remove","pdfa-remove",zt)),Q([i,c],e,n,"exporting")}function zt(){M(!0),h("Removing this viewer..."),b({action:"removeViewer",attachmentUUID:t.attachmentUUID,pluginUUID:t.pluginUUID}).then(function(e){if(!e||e.error)throw new Error(e&&e.error||"Could not remove this viewer.");document.body.innerHTML='<div style="padding:16px;font:13px sans-serif;opacity:.75">Removed - this block will disappear once the note refreshes.</div>'}).catch(function(e){h(e.message||String(e),!0)})}function qt(){o.pdfBytes&&(h("Preparing the download..."),Rt().then(function(e){return a.writeHighlightsIntoPdf(e,o.pdfBytes,o.highlights,Lt())}).then(function(e){var n=new Blob([e],{type:"application/pdf"}),i=URL.createObjectURL(n),c=document.createElement("a");c.href=i,c.download=Mt(),document.body.appendChild(c),c.click(),c.remove(),setTimeout(function(){URL.revokeObjectURL(i)},4e3),h("")}).catch(function(e){h("Could not prepare the download: "+(e.message||e),!0)}))}function Gt(){return b({action:"loadHighlights",attachmentUUID:t.attachmentUUID}).then(function(e){if(!e||e.error)throw new Error(e&&e.error||"No answer from the plugin");o.highlights=e.highlights||[]}).catch(function(e){o.highlights=[],h("Could not load saved highlights: "+(e.message||e),!0)})}function Vt(){var e=o.highlights.length;r.collapsedCount.textContent=e?e+(e===1?" highlight":" highlights"):"",r.root.classList.add("pdfa-collapsed-mode"),ze(!0)}function ze(e){b({action:"setCollapsed",collapsed:e,attachmentUUID:t.attachmentUUID,pluginUUID:t.pluginUUID}).catch(function(){})}function Wt(){r.root.classList.remove("pdfa-collapsed-mode"),o.doc||qe(),ze(!1)}function qe(){h("Loading PDF..."),(t.highlightId||t.page)&&It(),Ht().then(function(e){return e.GlobalWorkerOptions.workerSrc=t.workerSrc,b({action:"getPdfUrl",attachmentUUID:t.attachmentUUID})}).then(function(e){if(!e||!e.url)throw new Error(e&&e.error||"Could not resolve the PDF URL");return d(e.name),fetch(e.url)}).then(function(e){if(!e.ok)throw new Error("Download failed (HTTP "+e.status+")");return e.arrayBuffer()}).then(function(e){return o.pdfBytes=e.slice(0),window.pdfjsLib.getDocument({data:e}).promise}).then(function(e){return o.doc=e,o.pageCount=e.numPages,Gt()}).then(function(){return Dt()}).then(function(){return D()}).then(function(){O();var e=t.highlightId?S(t.highlightId):null;e?(Fe(e),Pt(e)):t.page&&xe(t.page)}).catch(function(e){h(e.message||String(e),!0)})}try{document.getElementById("pdfa-prev").onclick=function(){xe(o.current-1)},document.getElementById("pdfa-next").onclick=function(){xe(o.current+1)},document.getElementById("pdfa-zoom-in").onclick=function(){Oe(o.scale+.25)},document.getElementById("pdfa-zoom-out").onclick=function(){Oe(o.scale-.25)},r.scrollUp.onclick=function(){$e(-1)},r.scrollDown.onclick=function(){$e(1)},r.listToggle.onclick=function(){ae()},r.more.onclick=function(e){_t(e.clientX,e.clientY)},V().addEventListener("scroll",Ut),r.pages.addEventListener("mouseup",X),r.pages.addEventListener("click",kt),document.addEventListener("selectionchange",Ct),r.pages.addEventListener("touchend",function(){Y&&clearTimeout(Y),Y=null,He()}),document.addEventListener("keydown",function(e){e.key==="Escape"&&!o.noteEditing&&M()}),document.addEventListener("mousedown",function(e){r.popover.classList.contains("pdfa-open")&&(r.popover.contains(e.target)||M())}),u(),J(),r.root.querySelector(".pdfa-collapsed").onclick=Wt,t.collapsed?b({action:"getViewerSummary",attachmentUUID:t.attachmentUUID}).then(function(e){if(!(!e||e.error)){d(e.name);var n=e.count||0;r.collapsedCount.textContent=n?n+(n===1?" highlight":" highlights"):""}}).catch(function(){}):qe()}catch(e){h("Viewer failed to start: "+(e&&e.message?e.message:e),!0)}}function un(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function fn(t){return JSON.stringify(t).replace(/</g,"\\u003c")}var gn=`
...
`,xt={light:"--pdfa-bg:#f6f7f9; --pdfa-fg:#1c1e21; --pdfa-toolbar:#fff; --pdfa-border:#d8dbe0; --pdfa-btn:#fff; --pdfa-btn-hover:#eceef1; --pdfa-error:#b3261e; --pdfa-accent:#1a6fb5;",dark:"--pdfa-bg:#1e2126; --pdfa-fg:#e6e8ea; --pdfa-toolbar:#252930; --pdfa-border:#3a3f47; --pdfa-btn:#2d323a; --pdfa-btn-hover:#3a4049; --pdfa-error:#f2b8b5; --pdfa-accent:#79b8ef;"};function bt({attachmentUUID:t,attachmentName:s="",page:a=null,highlightId:l=null,lightDarkMode:r="light",pluginUUID:d=null,noteUUID:o=null,collapsed:h=!1}={}){let b=xt[r]||xt.light,m={attachmentUUID:t,page:a,highlightId:l,pluginUUID:d,noteUUID:o,pdfJsSrc:ne.pdfJs,workerSrc:ne.pdfJsWorker,pdfLibSrc:ne.pdfLib,colors:ie.map(T=>({id:T.id,label:T.label,hex:T.hex,rgb:T.rgb,cycleIndex:T.cycleIndex})),defaultColorId:se,collapsed:h,attachmentName:s};return`<link rel="stylesheet" href="${ne.pdfViewerCss}">
<style>:root{${b}}${gn}</style>
<div id="pdfa-root"${h?' class="pdfa-collapsed-mode"':""}>
  <div class="pdfa-collapsed">
    ...
    <span class="pdfa-collapsed-name">${un(s)}</span>
...
<script>window.__PDFA_CONFIG = ${fn(m)};
window.__PDFA_GEOM = (${ke.toString()})();
window.__PDFA_ANNOTATIONS = (${Ie.toString()})();
window.__PDFA_EXPORT = (${De.toString()})();<\/script>
<script>(${wt.toString()})();<\/script>`}var mn={noteOption:{"Annotate PDF":async function(t,s){return nt(t,s,t.context.pluginUUID)},"Debug: dump note markdown":async function(t,s){return ot(t,s)}},insertText:async function(t){return at(t,t.context.noteUUID,t.context.pluginUUID)},linkTarget:async function(t,...s){return rt(t,s[0])},renderEmbed:function(t,...s){let{attachmentUUID:a,page:l,highlightId:r,collapsed:d,attachmentName:o}=oe(s[0]);return a?bt({attachmentUUID:a,page:l,highlightId:r,collapsed:d,attachmentName:o,lightDarkMode:t.context.lightDarkMode,pluginUUID:t.context.pluginUUID,noteUUID:t.context.noteUUID}):`<p style="font:13px sans-serif;padding:12px">
        This viewer isn't linked to a PDF. Re-run <b>Annotate PDF</b> from the note menu.
      </p>`},onEmbedCall:async function(t,...s){return vt(t,s[0])}},vn=mn;return Zt(wn);})();
