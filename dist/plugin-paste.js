...
`)}function Ne(t,s,r,c={}){if(!t||!s||!r)return null;let i=t.split(`
`),d=`plugin://${s}`,a=i.findIndex(T=>T.includes(d)&&T.includes(`att=${r}`));if(a===-1)return null;let p=i[a],A=p.match(/data="(plugin:\/\/[^"]*)"/);if(!A)return null;let g=A[1],S=g.indexOf("?"),N=S===-1?"":g.slice(S+1),C={...ae(N),attachmentUUID:r,...c},u=nt(C),f=u?`plugin://${s}?${u}`:`plugin://${s}`,x=i.slice(),b=p.replace(A[0],`data="${f}"`),E=tt(C.collapsed),y=b.match(/data-aspect-ratio="[^"]*"/);return b=y?b.replace(y[0],`data-aspect-ratio="${E}"`):b.replace(/\s*\/>\s*$/,` data-aspect-ratio="${E}" />`),x[a]=b,x.join(`
`)}function at(t,s,r,c){return Ne(t,s,r,{collapsed:!!c})}async function rt(t,s,r){let c=await ce(t,s);if(!c){let p=await t.getNoteAttachments({uuid:s});return(!(Array.isArray(p)&&p.length>0)||!p.some(g=>g&&g.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.
...
`),c=Ae(r,G);if(!c)return null;let i=r.slice(0,c.start).join(`
...
`),c=Ae(r,G);if(!c)return null;let i=mn(r.slice(c.start+1,c.end).join(`
