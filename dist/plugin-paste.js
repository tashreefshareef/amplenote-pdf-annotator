...
var __pluginModule=(()=>{var Se=Object.defineProperty;var nn=Object.getOwnPropertyDescriptor;var on=Object.getOwnPropertyNames;var rn=Object.prototype.hasOwnProperty;var an=(t,s)=>{for(var r in s)Se(t,r,{get:s[r],enumerable:!0})},sn=(t,s,r,l)=>{if(s&&typeof s=="object"||typeof s=="function")for(let i of on(s))!rn.call(t,i)&&i!==r&&Se(t,i,{get:()=>s[i],enumerable:!(l=nn(s,i))||l.enumerable});return t};var ln=t=>sn(Se({},"__esModule",{value:!0}),t);var Tn={};an(Tn,{default:()=>Sn});var le=[{id:"coral",label:"Coral",hex:"#F3998C",cycleIndex:12,rgb:[.953,.6,.549]},{id:"yellow",label:"Yellow",hex:"#F4DE6C",cycleIndex:14,rgb:[.957,.871,.424]},{id:"green",label:"Green",hex:"#BBE077",cycleIndex:15,rgb:[.733,.878,.467]},{id:"blue",label:"Blue",hex:"#84B6D9",cycleIndex:18,rgb:[.518,.714,.851]}],ce="yellow",G="PDF Annotator data",Ye="attachment://",Qe=1,Ke=16,oe={pdfJs:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js",pdfJsWorker:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js",pdfViewerCss:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf_viewer.min.css",pdfLib:"https://cdnjs.cloudflare.com/ajax/libs/pdf-lib/1.17.1/pdf-lib.min.js"},cn="https://plugins.amplenote.com/cors-proxy";function Ze(t){let s=new URL(cn);return s.searchParams.set("apiurl",t),s.toString()}var dn="application/pdf";function hn(t){return Array.isArray(t)?t.filter(s=>s&&s.type===dn&&s.uuid):[]}async function de(t,s){let r=await t.getNoteAttachments({uuid:s}),l=hn(r);if(l.length===0)return null;if(l.length===1)return l[0];let i=await t.prompt("Which PDF do you want to annotate?",{inputs:[{label:"PDF",type:"radio",options:l.map(o=>({label:o.name,value:o.uuid})),value:l[0].uuid}]});if(i==null)return null;let d=Array.isArray(i)?i[0]:i;return l.find(o=>o.uuid===d)||null}async function et(t,s){if(!s)throw new Error("fetchableAttachmentURL: attachmentUUID required");let r=await t.getAttachmentURL(s);if(!r)throw new Error(`No URL returned for attachment ${s}`);return Ze(r)}function tt(t){return t?Ke:Qe}function re(t){let s={attachmentUUID:null,page:null,x:null,y:null,highlightId:null,noteUUID:null,collapsed:!1,attachmentName:""};if(!t||typeof t!="string")return s;let r;try{r=new URLSearchParams(t.replace(/^\?/,""))}catch{return s}let l=d=>{let o=r.get(d);if(o===null||o.trim()==="")return null;let p=Number(o);return Number.isFinite(p)?p:null},i=l("page");return{attachmentUUID:r.get("att")||null,page:i!==null&&i>=1?Math.floor(i):null,x:l("x"),y:l("y"),highlightId:r.get("hl")||null,noteUUID:r.get("note")||null,collapsed:r.get("c")==="1",attachmentName:r.get("n")||""}}function nt({attachmentUUID:t,page:s,x:r,y:l,highlightId:i,collapsed:d,attachmentName:o}={}){let p=new URLSearchParams;return t&&p.set("att",t),d&&p.set("c","1"),o&&p.set("n",o),Number.isFinite(s)&&s>=1&&p.set("page",String(Math.floor(s))),Number.isFinite(r)&&p.set("x",String(r)),Number.isFinite(l)&&p.set("y",String(l)),i&&p.set("hl",i),p.toString()}function he(t,s={},r=null){if(!t)throw new Error("buildEmbedMarkup: pluginUUID required");r===null&&(r=tt(s.collapsed));let l=nt(s);return`<object data="${l?`plugin://${t}?${l}`:`plugin://${t}`}" data-aspect-ratio="${r}" />`}function ot(t,s,r){if(!t||!s||!r)return null;let l=t.split(`
`),i=l.findIndex(o=>o.includes(`${Ye}${s}`));if(i===-1)return null;let d=l.slice();return l[i+1]===""?d.splice(i+2,0,r.trim(),""):d.splice(i+1,0,"",r.trim(),""),d.join(`
`)}function pe(t,s,r=null){return!t||!s||!t.includes(`plugin://${s}`)?!1:r?t.includes(`att=${r}`):!0}function ue(t,s,r){if(!t||!s||!r)return null;let l=t.split(`
`),i=`plugin://${s}`,d=l.findIndex(p=>p.includes(i)&&p.includes(`att=${r}`));if(d===-1)return null;let o=l.slice();return o.splice(d,1),o[d]===""&&o[d-1]===""&&o.splice(d,1),o.join(`
`)}function ae(t,s,r,l={}){if(!t||!s||!r)return null;let i=t.split(`
`),d=`plugin://${s}`,o=i.findIndex(T=>T.includes(d)&&T.includes(`att=${r}`));if(o===-1)return null;let p=i[o],k=p.match(/data="(plugin:\/\/[^"]*)"/);if(!k)return null;let S=k[1],y=S.indexOf("?"),N=y===-1?"":S.slice(y+1),v={...re(N),attachmentUUID:r,...l},u=nt(v),f=u?`plugin://${s}?${u}`:`plugin://${s}`,b=i.slice(),x=p.replace(k[0],`data="${f}"`),C=tt(v.collapsed),E=x.match(/data-aspect-ratio="[^"]*"/);return x=E?x.replace(E[0],`data-aspect-ratio="${C}"`):x.replace(/\s*\/>\s*$/,` data-aspect-ratio="${C}" />`),b[o]=x,b.join(`
`)}function rt(t,s,r,l){return ae(t,s,r,{collapsed:!!l})}async function at(t,s,r){let l=await de(t,s);if(!l){let p=await t.getNoteAttachments({uuid:s});return(!(Array.isArray(p)&&p.length>0)||!p.some(S=>S&&S.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.

Attach a PDF to this note, then run this again.`),null}let i=await t.getNoteContent({uuid:s});if(pe(i,r,l.uuid))return await t.alert(`"${l.name}" is already open in this note - scroll to the viewer.`),l.uuid;let d=he(r,{attachmentUUID:l.uuid,attachmentName:l.name}),o=ot(i,l.uuid,d);return o!==null?(await t.replaceNoteContent({uuid:s},o),l.uuid):(await t.insertNoteContent({uuid:s},`
${d}
`,{atEnd:!0}),l.uuid)}var pn="Raw markdown";function un(t){let s=(String(t||"").match(/`+/g)||[]).reduce((r,l)=>Math.max(r,l.length),0);return"`".repeat(Math.max(3,s+1))}async function it(t,s){let r=await t.getNoteContent({uuid:s});if(typeof r!="string"||r==="")return await t.alert("That note came back empty - nothing to dump."),null;let l=await t.getNoteAttachments({uuid:s}),i=(Array.isArray(l)?l:[]).map(p=>`- ${p&&p.name} | ${p&&p.type} | ${p&&p.uuid}`).join(`
...
Open it and look for how the PDF attachment is referenced.`),o}async function st(t,s,r){if(!s)return"";let l=await de(t,s);if(!l){let d=await t.getNoteAttachments({uuid:s});return(!(Array.isArray(d)&&d.length>0)||!d.some(p=>p&&p.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.

Attach a PDF to this note, then type {PDF Annotator} again where you want the viewer.`),""}let i=await t.getNoteContent({uuid:s});return pe(i,r,l.uuid)?(await t.alert(`"${l.name}" already has a viewer in this note.

Detach that one first if you want to move it here.`),""):`
${he(r,{attachmentUUID:l.uuid,attachmentName:l.name})}
`}async function fn(t,s,r,l){let i={uuid:s},d=ue(r,t.context.pluginUUID,l);if(d!==null)try{await t.replaceNoteContent(i,d)}catch{}try{await t.replaceNoteContent(i,r)}catch{await t.replaceNoteContent(i,r)}}async function lt(t,s){let{noteUUID:r,attachmentUUID:l,page:i,highlightId:d}=re(s);if(!r){await t.alert("This link doesn't say which note the PDF lives on - it was likely exported by an older version of this plugin. Re-export the highlight to get a working link.");return}try{let o=await t.getNoteContent({uuid:r}),p=ae(o,t.context.pluginUUID,l,{page:i,highlightId:d,collapsed:!1});p!==null&&(t.context&&t.context.noteUUID===r?await fn(t,r,p,l):await t.replaceNoteContent({uuid:r},p))}catch{}await t.navigate(`https://www.amplenote.com/notes/${r}`)}function fe(t){if(!t)return null;let s=String(t).trim().toLowerCase();return le.find(r=>r.id===s||r.hex.toLowerCase()===s)||null}function ct(){return fe(ce)}function gn(){return"hl-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2,8)}function ge({page:t,color:s,rects:r,quoteText:l,note:i=null,id:d=null}){if(!Number.isInteger(t)||t<1)throw new Error(`createHighlight: page must be a positive integer, got ${t}`);if(!Array.isArray(r)||r.length===0)throw new Error("createHighlight: rects must be a non-empty array");for(let p of r)if(![p.x,p.y,p.width,p.height].every(Number.isFinite))throw new Error(`createHighlight: malformed rect ${JSON.stringify(p)}`);let o=fe(s)||ct();return{id:d||gn(),page:t,color:o.id,rects:r.map(p=>({x:p.x,y:p.y,width:p.width,height:p.height})),quoteText:String(l||""),note:i?String(i):null}}function dt(t,s){let r=s==null?null:String(s).trim();return{...t,note:r||null}}function ht(t,s){let r=fe(s);if(!r)throw new Error(`withColor: unknown color "${s}"`);return{...t,color:r.id}}function pt(t,s){return(t||[]).filter(r=>r.id!==s)}function Te(t,s,r){let l=!1,i=(t||[]).map(d=>d.id!==s?d:(l=!0,r(d)));return l?i:t}var mn="json",ut="*Managed automatically by the PDF Annotator plugin - safe to ignore, don't edit.*";function ft(t){let s=JSON.stringify(t,null,0).replace(/`/g,"\\u0060");return`${ut}
\`\`\`${mn}
${s}
\`\`\``}function ke(t){if(!t)return null;let s=t.match(/```(?:json)?\s*\n?([\s\S]*?)\n?```/),r=!s&&t.match(/<!--\s*PDFA-DATA\s*\n?([\s\S]*?)-->/),l=(s?s[1]:r?r[1]:t).trim();if(!l)return null;try{return JSON.parse(l)}catch{return null}}function vn(t){if(!Array.isArray(t))return[];let s=[];for(let r of t)try{s.push(ge(r))}catch{}return s}async function me(t,s,r){let l=await t.getNoteContent({uuid:s}),i=Ie(l,G),d=ke(i);return!d||typeof d!="object"?[]:vn(d[r])}async function gt(t,s,r,l){let i={uuid:s},d=await t.getNoteContent(i),o=Ie(d,G),k={...ke(o)||{},[r]:l},S=ft(k);o===null&&await t.insertNoteContent(i,`

# ${G}

`,{atEnd:!0});let y=bn(d,S);if(y!==null){await t.replaceNoteContent(i,y);return}await t.replaceNoteContent(i,S,{section:{heading:{text:G,level:1}}})}async function mt(t,s,r){let l={uuid:s},i=await t.getNoteContent(l),d=Ie(i,G);if(d===null)return;let o=ke(d)||{};if(!(r in o))return;let p={...o};delete p[r],await t.replaceNoteContent(l,ft(p),{section:{heading:{text:G,level:1}}})}function Ae(t,s){let r=/^#\s+(.*)$/,l=t.findIndex(d=>{let o=d.match(r);return o&&o[1].trim()===s});if(l===-1)return null;let i=t.length;for(let d=l+1;d<t.length;d++)if(/^#\s+/.test(t[d])){i=d;break}return{start:l,end:i}}function Ie(t,s){if(!t)return null;let r=t.split(`
`),l=Ae(r,s);return l?r.slice(l.start+1,l.end).join(`
`).trim():null}function wn(t){if(!t)return"";let s=t,r=s.match(/```(?:json)?\s*\n?[\s\S]*?\n?```/);return r&&(s=s.replace(r[0],"")),s=s.replace(/<!--\s*PDFA-DATA[\s\S]*?-->/,""),s=s.replace(ut,""),s.trim()}function vt(t,s){return String(t||"").includes("](plugin://")?s:`---

${s}`}function wt(t,s){let r=(t||"").split(`
`),l=Ae(r,G);if(!l)return null;let i=r.slice(0,l.start).join(`
`).replace(/\s+$/,""),d=r.slice(l.start).join(`
`);return`${i?i+`

`:""}${s}

${d}`}function bn(t,s){let r=(t||"").split(`
`),l=Ae(r,G);if(!l)return null;let i=wn(r.slice(l.start+1,l.end).join(`
`).trim());if(!i)return null;let d=r.slice(0,l.start).join(`
`).replace(/\s+$/,""),o=r.slice(l.end).join(`
...
`+o:""}`}function q(t,s){return s.noteUUID||t.context.noteUUID}async function bt(t,s,r){try{let l=await t.getNoteAttachments({uuid:s}),i=Array.isArray(l)&&l.find(d=>d&&d.uuid===r);return i?i.name:""}catch{return""}}async function ve(t,s,r,l){let i=await me(t,s,r),d=l(i);return d!==i&&await gt(t,s,r,d),{highlights:d}}function xt(t){if(t&&typeof t=="object")return t;if(typeof t!="string")return{};let s=t.trim();if(!s.startsWith("{"))return{action:s};try{return JSON.parse(s)}catch{return{action:s}}}async function yt(t,s){return JSON.stringify(await xn(t,xt(s)))}async function xn(t,s){let r=xt(s);switch(r.action){case"getPdfUrl":{let l=r.attachmentUUID;if(!l)return{error:"No attachment specified for this viewer."};try{return{url:await et(t,l),name:await bt(t,q(t,r),l)}}catch(i){return{error:`Could not load the PDF: ${i.message}`}}}case"loadHighlights":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return{highlights:await me(t,q(t,r),r.attachmentUUID)}}catch(l){return{error:`Could not load highlights: ${l.message}`}}}case"addHighlight":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};try{let l=ge(r.highlight||{});return await ve(t,q(t,r),r.attachmentUUID,i=>i.concat([l]))}catch(l){return{error:`Could not save the highlight: ${l.message}`}}}case"recolorHighlight":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ve(t,q(t,r),r.attachmentUUID,l=>Te(l,r.id,i=>ht(i,r.color)))}catch(l){return{error:`Could not change the highlight color: ${l.message}`}}}case"setHighlightNote":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ve(t,q(t,r),r.attachmentUUID,l=>Te(l,r.id,i=>dt(i,r.note)))}catch(l){return{error:`Could not save the note: ${l.message}`}}}case"removeHighlight":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ve(t,q(t,r),r.attachmentUUID,l=>pt(l,r.id))}catch(l){return{error:`Could not remove the highlight: ${l.message}`}}}case"sendToNote":{if(!r.content)return{error:"Nothing to send."};try{let l={uuid:q(t,r)},i=await t.getNoteContent(l),d=vt(i,r.content),o=wt(i,d);return o===null?await t.insertNoteContent(l,`
`+d+`
`,{atEnd:!0}):await t.replaceNoteContent(l,o),{ok:!0}}catch(l){return{error:`Could not add this to the note: ${l.message}`}}}case"removeViewer":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!r.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=q(t,r),i=await t.getNoteContent({uuid:l}),d=ue(i,r.pluginUUID,r.attachmentUUID);return d===null?{error:"Could not find this viewer's block in the note - it may already be removed."}:(await t.replaceNoteContent({uuid:l},d),await mt(t,l,r.attachmentUUID),{ok:!0})}catch(l){return{error:`Could not remove this viewer: ${l.message}`}}}case"getViewerSummary":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};let l=q(t,r),i=await bt(t,l,r.attachmentUUID);try{let d=await me(t,l,r.attachmentUUID);return{name:i,count:d.length}}catch{return{name:i,count:0}}}case"setCollapsed":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!r.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=q(t,r),i=await t.getNoteContent({uuid:l}),d=rt(i,r.pluginUUID,r.attachmentUUID,r.collapsed);return d===null?{ok:!1}:(await t.replaceNoteContent({uuid:l},d),{ok:!0})}catch(l){return{error:`Could not resize this viewer: ${l.message}`}}}case"clearDeepLink":{if(!r.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!r.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=q(t,r),i=await t.getNoteContent({uuid:l}),d=ae(i,r.pluginUUID,r.attachmentUUID,{page:null,highlightId:null});return d===null?{ok:!1}:(await t.replaceNoteContent({uuid:l},d),{ok:!0})}catch(l){return{error:`Could not clear this viewer's deep link: ${l.message}`}}}case"exportAll":{if(!r.noteName)return{error:"Missing destination note name."};try{let l=await t.findNote({name:r.noteName}),i=l?l.uuid:await t.createNote(r.noteName);return await t.replaceNoteContent({uuid:i},r.content||""),{ok:!0,noteUUID:i}}catch(l){return{error:`Could not export highlights: ${l.message}`}}}case"ping":return{ok:!0};default:return{error:`Unknown embed action: ${String(r.action)}`}}}function De(){function t(u,f){return{x:u.left-f.left,y:u.top-f.top,width:u.width,height:u.height}}function s(u,f){return{x:Math.min(u[0],f[0]),y:Math.min(u[1],f[1]),width:Math.abs(f[0]-u[0]),height:Math.abs(f[1]-u[1])}}function r(u,f){var b=Math.pow(10,f===void 0?2:f),x=function(C){return Math.round(C*b)/b};return{x:x(u.x),y:x(u.y),width:x(u.width),height:x(u.height)}}function l(u){return u.width>.01&&u.height>.01}function i(u,f,b){for(var x=String(u??""),C=Math.max(0,f===void 0?0:f),E=Math.min(x.length,b===void 0?x.length:b),T=function(O){return O===""||/\s/.test(O)},A=[],I=C;I<E;){for(;I<E&&T(x.charAt(I));)I++;if(I>=E)break;for(var $=I;I<E&&!T(x.charAt(I));)I++;A.push({start:$,end:I})}return A}function d(u){for(var f=1/0,b=1/0,x=-1/0,C=-1/0,E=0;E<(u?u.length:0);E++){var T=u[E];l(T)&&(f=Math.min(f,T.left),b=Math.min(b,T.top),x=Math.max(x,T.left+T.width),C=Math.max(C,T.top+T.height))}return isFinite(f)?{left:f,top:b,width:x-f,height:C-b}:null}function o(u,f,b){for(var x=[],C=0;C<u.length;C++){var E=t(u[C],f);if(l(E)){var T=b(E.x,E.y),A=b(E.x+E.width,E.y+E.height),I=r(s(T,A));l(I)&&x.push(I)}}return x}function p(u,f){var b=f(u.x,u.y),x=f(u.x+u.width,u.y+u.height);return s(b,x)}function k(u,f,b){var x=f.right-f.left,C=f.bottom-f.top;if(x<=0||C<=0)return null;var E=u.x2-u.x1,T=u.y2-u.y1,A=u.x1+(b.left-f.left)/x*E,I=u.x2-(f.right-b.right)/x*E,$=u.y1+(b.bottom-f.bottom)/C*T,O=u.y2-(f.top-b.top)/C*T;return{x:A,y:$,width:I-A,height:O-$}}function S(u,f){var b=Math.min(u.y+u.height,f.y+f.height)-Math.max(u.y,f.y);return b>.5*Math.min(u.height,f.height)}function y(u,f){var b=f===void 0?.6:f;if(!u||u.length<2)return(u||[]).slice();for(var x=u.slice().sort(function(j,J){return J.y-j.y||j.x-J.x}),C=[],E=0;E<x.length;E++){for(var T=!1,A=0;A<C.length;A++)if(S(C[A][0],x[E])){C[A].push(x[E]),T=!0;break}T||C.push([x[E]])}for(var I=[],$=0;$<C.length;$++){for(var O=C[$].slice().sort(function(j,J){return j.x-J.x}),R=null,W=0;W<O.length;W++){var F=O[W];if(R===null){R={x:F.x,y:F.y,width:F.width,height:F.height};continue}var ie=F.x-(R.x+R.width);if(ie<=b*Math.max(R.height,F.height)){var ee=Math.max(R.x+R.width,F.x+F.width),be=Math.max(R.y+R.height,F.y+F.height);R.x=Math.min(R.x,F.x),R.y=Math.min(R.y,F.y),R.width=ee-R.x,R.height=be-R.y}else I.push(R),R={x:F.x,y:F.y,width:F.width,height:F.height}}R!==null&&I.push(R)}return I.map(function(j){return r(j)})}function N(u,f,b,x){var C=x===void 0?0:x;return f>=u.x-C&&f<=u.x+u.width+C&&b>=u.y-C&&b<=u.y+u.height+C}function w(u,f,b,x,C){for(var E=u||[],T=E.length-1;T>=0;T--){var A=E[T];if(!(!A||A.page!==f||!A.rects)){for(var I=0;I<A.rects.length;I++)if(N(A.rects[I],b,x,C===void 0?1:C))return A}}return null}function v(u){return String(u??"").replace(/\s+/g," ").trim()}return{clientRectToLocal:t,rectFromCorners:s,roundRect:r,isVisibleRect:l,textTokenRanges:i,unionClientRects:d,clientRectsToPdfRects:o,pdfRectToViewportRect:p,itemRelativeRect:k,mergeLineRects:y,rectContainsPoint:N,hitTestHighlights:w,normalizeQuoteText:v}}var _=De(),eo=_.clientRectToLocal,to=_.rectFromCorners,no=_.roundRect,oo=_.isVisibleRect,ro=_.textTokenRanges,ao=_.unionClientRects,io=_.clientRectsToPdfRects,so=_.pdfRectToViewportRect,lo=_.itemRelativeRect,co=_.mergeLineRects,ho=_.rectContainsPoint,po=_.hitTestHighlights,uo=_.normalizeQuoteText;function Ue(){var t=[.957,.871,.424];function s(d,o,p,k,S){var y=o.context.register(o.context.obj({Type:d.PDFName.of("ExtGState"),BM:d.PDFName.of("Multiply"),ca:d.PDFNumber.of(.4)})),N=[d.pushGraphicsState(),d.setGraphicsState("GS0")];N.push(d.setFillingColor(d.rgb(k[0],k[1],k[2])));for(var w=0;w<p.length;w++){var v=p[w];N.push(d.moveTo(v.x,v.y)),N.push(d.lineTo(v.x,v.y+v.height)),N.push(d.lineTo(v.x+v.width,v.y+v.height)),N.push(d.lineTo(v.x+v.width,v.y)),N.push(d.closePath())}N.push(d.fill()),N.push(d.popGraphicsState());var u=o.context.formXObject(N,{BBox:S,Resources:{ExtGState:{GS0:y}}});return o.context.register(u)}function r(d,o,p,k){for(var S=p.rects,y=[],N=S[0].x,w=S[0].y,v=S[0].x+S[0].width,u=S[0].y+S[0].height,f=0;f<S.length;f++){var b=S[f],x=b.x,C=b.x+b.width,E=b.y,T=b.y+b.height;y.push(x,T,C,T,x,E,C,E),N=Math.min(N,x),w=Math.min(w,E),v=Math.max(v,C),u=Math.max(u,T)}var A=o.context.obj({Type:d.PDFName.of("Annot"),Subtype:d.PDFName.of("Highlight"),Rect:o.context.obj([N,w,v,u]),QuadPoints:o.context.obj(y),C:o.context.obj(k),F:d.PDFNumber.of(4),T:d.PDFString.of("PDF Annotator"),M:d.PDFString.of(new Date().toISOString()),CA:d.PDFNumber.of(.4)});p.note&&A.set(d.PDFName.of("Contents"),d.PDFString.of(p.note));var I=s(d,o,S,k,[N,w,v,u]);A.set(d.PDFName.of("AP"),o.context.obj({N:I}));var $=o.context.register(A),O=[$];if(p.note){var R=o.context.register(o.context.obj({Type:d.PDFName.of("Annot"),Subtype:d.PDFName.of("Popup"),Rect:o.context.obj([v+8,w-60,v+208,w+12]),Parent:$,Open:!1}));A.set(d.PDFName.of("Popup"),R),O.push(R)}return O}function l(d,o,p){var k=o.node.get(d.PDFName.of("Annots"));if(k instanceof d.PDFArray)for(var S=0;S<p.length;S++)k.push(p[S]);else o.node.set(d.PDFName.of("Annots"),o.doc.context.obj(p))}async function i(d,o,p,k){for(var S=await d.PDFDocument.load(o),y=S.getPages(),N=p||[],w=0;w<N.length;w++){var v=N[w];if(!(!v||!v.rects||!v.rects.length)){var u=y[v.page-1];if(u){var f=k&&k[v.color]||t,b=r(d,S,v,f);l(d,u,b)}}}return S.save()}return{writeHighlightsIntoPdf:i,buildHighlightAnnotation:r,appendAnnotationRefs:l}}var Pe=Ue(),go=Pe.writeHighlightsIntoPdf,mo=Pe.buildHighlightAnnotation,vo=Pe.appendAnnotationRefs;function He(){function t(y){return String(y??"").replace(/\]/g,"\\]").replace(/</g,"&lt;")}function s(y,N,w,v,u){var f=new URLSearchParams;N&&f.set("att",N),Number.isFinite(w)&&w>=1&&f.set("page",String(Math.floor(w))),v&&f.set("hl",v),u&&f.set("note",u);var b=f.toString();return"plugin://"+y+(b?"?"+b:"")}function r(y,N){return String(y??"").split(/\r?\n/).map(function(w){return(N+" "+w).replace(/[ \t]+$/,"")})}function l(y,N,w){return N==null?y:"<mark"+(w?' style="background-color:'+w+';"':"")+">"+y+'<!-- {"backgroundCycleColor":"'+N+'"} --></mark>'}function i(y,N,w,v,u,f,b){var x=s(N,w,v.page,v.id,b),C=l(t(y||"PDF"),u,f),E="["+C+"]("+x+")",T=[E].concat(r(v.quoteText,"> >"));return v.note&&(T.push(">"),T=T.concat(r(v.note,">"))),T.join(`
...
<script>(${Ct.toString()})();<\/script>`}var Nn={noteOption:{"Annotate PDF":async function(t,s){return at(t,s,t.context.pluginUUID)},"Debug: dump note markdown":async function(t,s){return it(t,s)}},insertText:async function(t){return st(t,t.context.noteUUID,t.context.pluginUUID)},linkTarget:async function(t,...s){return lt(t,s[0])},renderEmbed:function(t,...s){let{attachmentUUID:r,page:l,highlightId:i,collapsed:d,attachmentName:o}=re(s[0]);return r?Nt({attachmentUUID:r,page:l,highlightId:i,collapsed:d,attachmentName:o,lightDarkMode:t.context.lightDarkMode,pluginUUID:t.context.pluginUUID,noteUUID:t.context.noteUUID}):`<p style="font:13px sans-serif;padding:12px">
        This viewer isn't linked to a PDF. Re-run <b>Annotate PDF</b> from the note menu.
      </p>`},onEmbedCall:async function(t,...s){return yt(t,s[0])}},Sn=Nn;return ln(Tn);})();
