...
var __pluginModule=(()=>{var ye=Object.defineProperty;var $t=Object.getOwnPropertyDescriptor;var _t=Object.getOwnPropertyNames;var zt=Object.prototype.hasOwnProperty;var jt=(t,s)=>{for(var a in s)ye(t,a,{get:s[a],enumerable:!0})},Bt=(t,s,a,l)=>{if(s&&typeof s=="object"||typeof s=="function")for(let i of _t(s))!zt.call(t,i)&&i!==a&&ye(t,i,{get:()=>s[i],enumerable:!(l=$t(s,i))||l.enumerable});return t};var qt=t=>Bt(ye({},"__esModule",{value:!0}),t);var sn={};jt(sn,{default:()=>rn});var re=[{id:"coral",label:"Coral",hex:"#F3998C",cycleIndex:12,rgb:[.953,.6,.549]},{id:"yellow",label:"Yellow",hex:"#F4DE6C",cycleIndex:14,rgb:[.957,.871,.424]},{id:"green",label:"Green",hex:"#BBE077",cycleIndex:15,rgb:[.733,.878,.467]},{id:"blue",label:"Blue",hex:"#84B6D9",cycleIndex:18,rgb:[.518,.714,.851]}],ie="yellow",J="PDF Annotator data",Be="attachment://",qe=1,Ge=16,te={pdfJs:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js",pdfJsWorker:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js",pdfViewerCss:"https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf_viewer.min.css",pdfLib:"https://cdnjs.cloudflare.com/ajax/libs/pdf-lib/1.17.1/pdf-lib.min.js"},Gt="https://plugins.amplenote.com/cors-proxy";function Ve(t){let s=new URL(Gt);return s.searchParams.set("apiurl",t),s.toString()}var Vt="application/pdf";function Wt(t){return Array.isArray(t)?t.filter(s=>s&&s.type===Vt&&s.uuid):[]}async function se(t,s){let a=await t.getNoteAttachments({uuid:s}),l=Wt(a);if(l.length===0)return null;if(l.length===1)return l[0];let i=await t.prompt("Which PDF do you want to annotate?",{inputs:[{label:"PDF",type:"radio",options:l.map(n=>({label:n.name,value:n.uuid})),value:l[0].uuid}]});if(i==null)return null;let c=Array.isArray(i)?i[0]:i;return l.find(n=>n.uuid===c)||null}async function We(t,s){if(!s)throw new Error("fetchableAttachmentURL: attachmentUUID required");let a=await t.getAttachmentURL(s);if(!a)throw new Error(`No URL returned for attachment ${s}`);return Ve(a)}function Je(t){return t?Ge:qe}function ne(t){let s={attachmentUUID:null,page:null,x:null,y:null,highlightId:null,noteUUID:null,collapsed:!1,attachmentName:""};if(!t||typeof t!="string")return s;let a;try{a=new URLSearchParams(t.replace(/^\?/,""))}catch{return s}let l=c=>{let n=a.get(c);if(n===null||n.trim()==="")return null;let h=Number(n);return Number.isFinite(h)?h:null},i=l("page");return{attachmentUUID:a.get("att")||null,page:i!==null&&i>=1?Math.floor(i):null,x:l("x"),y:l("y"),highlightId:a.get("hl")||null,noteUUID:a.get("note")||null,collapsed:a.get("c")==="1",attachmentName:a.get("n")||""}}function Xe({attachmentUUID:t,page:s,x:a,y:l,highlightId:i,collapsed:c,attachmentName:n}={}){let h=new URLSearchParams;return t&&h.set("att",t),c&&h.set("c","1"),n&&h.set("n",n),Number.isFinite(s)&&s>=1&&h.set("page",String(Math.floor(s))),Number.isFinite(a)&&h.set("x",String(a)),Number.isFinite(l)&&h.set("y",String(l)),i&&h.set("hl",i),h.toString()}function le(t,s={},a=null){if(!t)throw new Error("buildEmbedMarkup: pluginUUID required");a===null&&(a=Je(s.collapsed));let l=Xe(s);return`<object data="${l?`plugin://${t}?${l}`:`plugin://${t}`}" data-aspect-ratio="${a}" />`}function Ye(t,s,a){if(!t||!s||!a)return null;let l=t.split(`
...
`)}function de(t,s,a=null){return!t||!s||!t.includes(`plugin://${s}`)?!1:a?t.includes(`att=${a}`):!0}function Ze(t,s,a){if(!t||!s||!a)return null;let l=t.split(`
...
Open it and look for how the PDF attachment is referenced.`),n}async function tt(t,s,a){if(!s)return"";let l=await se(t,s);if(!l){let c=await t.getNoteAttachments({uuid:s});return(!(Array.isArray(c)&&c.length>0)||!c.some(h=>h&&h.type==="application/pdf"))&&await t.alert(`No PDF attachments on this note.
...
`}async function nt(t,s){let{noteUUID:a,attachmentUUID:l,page:i,highlightId:c}=ne(s);if(!a){await t.alert("This link doesn't say which note the PDF lives on - it was likely exported by an older version of this plugin. Re-export the highlight to get a working link.");return}try{let n=await t.getNoteContent({uuid:a}),h=Ce(n,t.context.pluginUUID,l,{page:i,highlightId:c,collapsed:!1});h!==null&&await t.replaceNoteContent({uuid:a},h)}catch{}await t.navigate(`https://www.amplenote.com/notes/${a}`)}function ce(t){if(!t)return null;let s=String(t).trim().toLowerCase();return re.find(a=>a.id===s||a.hex.toLowerCase()===s)||null}function ot(){return ce(ie)}function Yt(){return"hl-"+Date.now().toString(36)+"-"+Math.random().toString(36).slice(2,8)}function he({page:t,color:s,rects:a,quoteText:l,note:i=null,id:c=null}){if(!Number.isInteger(t)||t<1)throw new Error(`createHighlight: page must be a positive integer, got ${t}`);if(!Array.isArray(a)||a.length===0)throw new Error("createHighlight: rects must be a non-empty array");for(let h of a)if(![h.x,h.y,h.width,h.height].every(Number.isFinite))throw new Error(`createHighlight: malformed rect ${JSON.stringify(h)}`);let n=ce(s)||ot();return{id:c||Yt(),page:t,color:n.id,rects:a.map(h=>({x:h.x,y:h.y,width:h.width,height:h.height})),quoteText:String(l||""),note:i?String(i):null}}function at(t,s){let a=s==null?null:String(s).trim();return{...t,note:a||null}}function rt(t,s){let a=ce(s);if(!a)throw new Error(`withColor: unknown color "${s}"`);return{...t,color:a.id}}function it(t,s){return(t||[]).filter(a=>a.id!==s)}function Ee(t,s,a){let l=!1,i=(t||[]).map(c=>c.id!==s?c:(l=!0,a(c)));return l?i:t}var Zt="json",Qt="*Managed automatically by the PDF Annotator plugin - safe to ignore, don't edit.*";function st(t){let s=JSON.stringify(t,null,0).replace(/`/g,"\\u0060");return`${Qt}
\`\`\`${Zt}
${s}
\`\`\``}function Ne(t){if(!t)return null;let s=t.match(/```(?:json)?\s*\n?([\s\S]*?)\n?```/),a=!s&&t.match(/<!--\s*PDFA-DATA\s*\n?([\s\S]*?)-->/),l=(s?s[1]:a?a[1]:t).trim();if(!l)return null;try{return JSON.parse(l)}catch{return null}}function Kt(t){if(!Array.isArray(t))return[];let s=[];for(let a of t)try{s.push(he(a))}catch{}return s}async function pe(t,s,a){let l=await t.getNoteContent({uuid:s}),i=Te(l,J),c=Ne(i);return!c||typeof c!="object"?[]:Kt(c[a])}async function lt(t,s,a,l){let i={uuid:s},c=await t.getNoteContent(i),n=Te(c,J),b={...Ne(n)||{},[a]:l},g=st(b);n===null&&await t.insertNoteContent(i,`

# ${J}

`,{atEnd:!0}),await t.replaceNoteContent(i,g,{section:{heading:{text:J,level:1}}})}async function dt(t,s,a){let l={uuid:s},i=await t.getNoteContent(l),c=Te(i,J);if(c===null)return;let n=Ne(c)||{};if(!(a in n))return;let h={...n};delete h[a],await t.replaceNoteContent(l,st(h),{section:{heading:{text:J,level:1}}})}function Te(t,s){if(!t)return null;let a=t.split(`
...
`).trim()}function j(t,s){return s.noteUUID||t.context.noteUUID}async function ct(t,s,a){try{let l=await t.getNoteAttachments({uuid:s}),i=Array.isArray(l)&&l.find(c=>c&&c.uuid===a);return i?i.name:""}catch{return""}}async function ue(t,s,a,l){let i=await pe(t,s,a),c=l(i);return c!==i&&await lt(t,s,a,c),{highlights:c}}function ht(t){if(t&&typeof t=="object")return t;if(typeof t!="string")return{};let s=t.trim();if(!s.startsWith("{"))return{action:s};try{return JSON.parse(s)}catch{return{action:s}}}async function pt(t,s){return JSON.stringify(await en(t,ht(s)))}async function en(t,s){let a=ht(s);switch(a.action){case"getPdfUrl":{let l=a.attachmentUUID;if(!l)return{error:"No attachment specified for this viewer."};try{return{url:await We(t,l),name:await ct(t,j(t,a),l)}}catch(i){return{error:`Could not load the PDF: ${i.message}`}}}case"loadHighlights":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return{highlights:await pe(t,j(t,a),a.attachmentUUID)}}catch(l){return{error:`Could not load highlights: ${l.message}`}}}case"addHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{let l=he(a.highlight||{});return await ue(t,j(t,a),a.attachmentUUID,i=>i.concat([l]))}catch(l){return{error:`Could not save the highlight: ${l.message}`}}}case"recolorHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ue(t,j(t,a),a.attachmentUUID,l=>Ee(l,a.id,i=>rt(i,a.color)))}catch(l){return{error:`Could not change the highlight color: ${l.message}`}}}case"setHighlightNote":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ue(t,j(t,a),a.attachmentUUID,l=>Ee(l,a.id,i=>at(i,a.note)))}catch(l){return{error:`Could not save the note: ${l.message}`}}}case"removeHighlight":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};try{return await ue(t,j(t,a),a.attachmentUUID,l=>it(l,a.id))}catch(l){return{error:`Could not remove the highlight: ${l.message}`}}}case"sendToNote":{if(!a.content)return{error:"Nothing to send."};try{return await t.insertNoteContent({uuid:j(t,a)},`
`+a.content+`
`,{atEnd:!0}),{ok:!0}}catch(l){return{error:`Could not add this to the note: ${l.message}`}}}case"removeViewer":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!a.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=j(t,a),i=await t.getNoteContent({uuid:l}),c=Ze(i,a.pluginUUID,a.attachmentUUID);return c===null?{error:"Could not find this viewer's block in the note - it may already be removed."}:(await t.replaceNoteContent({uuid:l},c),await dt(t,l,a.attachmentUUID),{ok:!0})}catch(l){return{error:`Could not remove this viewer: ${l.message}`}}}case"getViewerSummary":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};let l=j(t,a),i=await ct(t,l,a.attachmentUUID);try{let c=await pe(t,l,a.attachmentUUID);return{name:i,count:c.length}}catch{return{name:i,count:0}}}case"setCollapsed":{if(!a.attachmentUUID)return{error:"No attachment specified for this viewer."};if(!a.pluginUUID)return{error:"Missing plugin id - cannot locate this viewer."};try{let l=j(t,a),i=await t.getNoteContent({uuid:l}),c=Qe(i,a.pluginUUID,a.attachmentUUID,a.collapsed);return c===null?{ok:!1}:(await t.replaceNoteContent({uuid:l},c),{ok:!0})}catch(l){return{error:`Could not resize this viewer: ${l.message}`}}}case"exportAll":{if(!a.noteName)return{error:"Missing destination note name."};try{let l=await t.findNote({name:a.noteName}),i=l?l.uuid:await t.createNote(a.noteName);return await t.replaceNoteContent({uuid:i},a.content||""),{ok:!0,noteUUID:i}}catch(l){return{error:`Could not export highlights: ${l.message}`}}}case"ping":return{ok:!0};default:return{error:`Unknown embed action: ${String(a.action)}`}}}function Se(){function t(u,f){return{x:u.left-f.left,y:u.top-f.top,width:u.width,height:u.height}}function s(u,f){return{x:Math.min(u[0],f[0]),y:Math.min(u[1],f[1]),width:Math.abs(f[0]-u[0]),height:Math.abs(f[1]-u[1])}}function a(u,f){var x=Math.pow(10,f===void 0?2:f),v=function(C){return Math.round(C*x)/x};return{x:v(u.x),y:v(u.y),width:v(u.width),height:v(u.height)}}function l(u){return u.width>.01&&u.height>.01}function i(u,f,x){for(var v=String(u??""),C=Math.max(0,f===void 0?0:f),N=Math.min(v.length,x===void 0?v.length:x),A=function($){return $===""||/\s/.test($)},U=[],I=C;I<N;){for(;I<N&&A(v.charAt(I));)I++;if(I>=N)break;for(var O=I;I<N&&!A(v.charAt(I));)I++;U.push({start:O,end:I})}return U}function c(u){for(var f=1/0,x=1/0,v=-1/0,C=-1/0,N=0;N<(u?u.length:0);N++){var A=u[N];l(A)&&(f=Math.min(f,A.left),x=Math.min(x,A.top),v=Math.max(v,A.left+A.width),C=Math.max(C,A.top+A.height))}return isFinite(f)?{left:f,top:x,width:v-f,height:C-x}:null}function n(u,f,x){for(var v=[],C=0;C<u.length;C++){var N=t(u[C],f);if(l(N)){var A=x(N.x,N.y),U=x(N.x+N.width,N.y+N.height),I=a(s(A,U));l(I)&&v.push(I)}}return v}function h(u,f){var x=f(u.x,u.y),v=f(u.x+u.width,u.y+u.height);return s(x,v)}function b(u,f,x){var v=f.right-f.left,C=f.bottom-f.top;if(v<=0||C<=0)return null;var N=u.x2-u.x1,A=u.y2-u.y1,U=u.x1+(x.left-f.left)/v*N,I=u.x2-(f.right-x.right)/v*N,O=u.y1+(x.bottom-f.bottom)/C*A,$=u.y2-(f.top-x.top)/C*A;return{x:U,y:O,width:I-U,height:$-O}}function g(u,f){var x=Math.min(u.y+u.height,f.y+f.height)-Math.max(u.y,f.y);return x>.5*Math.min(u.height,f.height)}function k(u,f){var x=f===void 0?.6:f;if(!u||u.length<2)return(u||[]).slice();for(var v=u.slice().sort(function(G,W){return W.y-G.y||G.x-W.x}),C=[],N=0;N<v.length;N++){for(var A=!1,U=0;U<C.length;U++)if(g(C[U][0],v[N])){C[U].push(v[N]),A=!0;break}A||C.push([v[N]])}for(var I=[],O=0;O<C.length;O++){for(var $=C[O].slice().sort(function(G,W){return G.x-W.x}),D=null,Q=0;Q<$.length;Q++){var R=$[Q];if(D===null){D={x:R.x,y:R.y,width:R.width,height:R.height};continue}var oe=R.x-(D.x+D.width);if(oe<=x*Math.max(D.height,R.height)){var fe=Math.max(D.x+D.width,R.x+R.width),B=Math.max(D.y+D.height,R.y+R.height);D.x=Math.min(D.x,R.x),D.y=Math.min(D.y,R.y),D.width=fe-D.x,D.height=B-D.y}else I.push(D),D={x:R.x,y:R.y,width:R.width,height:R.height}}D!==null&&I.push(D)}return I.map(function(G){return a(G)})}function T(u,f,x,v){var C=v===void 0?0:v;return f>=u.x-C&&f<=u.x+u.width+C&&x>=u.y-C&&x<=u.y+u.height+C}function y(u,f,x,v,C){for(var N=u||[],A=N.length-1;A>=0;A--){var U=N[A];if(!(!U||U.page!==f||!U.rects)){for(var I=0;I<U.rects.length;I++)if(T(U.rects[I],x,v,C===void 0?1:C))return U}}return null}function E(u){return String(u??"").replace(/\s+/g," ").trim()}return{clientRectToLocal:t,rectFromCorners:s,roundRect:a,isVisibleRect:l,textTokenRanges:i,unionClientRects:c,clientRectsToPdfRects:n,pdfRectToViewportRect:h,itemRelativeRect:b,mergeLineRects:k,rectContainsPoint:T,hitTestHighlights:y,normalizeQuoteText:E}}var z=Se(),Ln=z.clientRectToLocal,Mn=z.rectFromCorners,Fn=z.roundRect,On=z.isVisibleRect,$n=z.textTokenRanges,_n=z.unionClientRects,zn=z.clientRectsToPdfRects,jn=z.pdfRectToViewportRect,Bn=z.itemRelativeRect,qn=z.mergeLineRects,Gn=z.rectContainsPoint,Vn=z.hitTestHighlights,Wn=z.normalizeQuoteText;function Ae(){var t=[.957,.871,.424];function s(c,n,h,b,g){var k=n.context.register(n.context.obj({Type:c.PDFName.of("ExtGState"),BM:c.PDFName.of("Multiply"),ca:c.PDFNumber.of(.4)})),T=[c.pushGraphicsState(),c.setGraphicsState("GS0")];T.push(c.setFillingColor(c.rgb(b[0],b[1],b[2])));for(var y=0;y<h.length;y++){var E=h[y];T.push(c.moveTo(E.x,E.y)),T.push(c.lineTo(E.x,E.y+E.height)),T.push(c.lineTo(E.x+E.width,E.y+E.height)),T.push(c.lineTo(E.x+E.width,E.y)),T.push(c.closePath())}T.push(c.fill()),T.push(c.popGraphicsState());var u=n.context.formXObject(T,{BBox:g,Resources:{ExtGState:{GS0:k}}});return n.context.register(u)}function a(c,n,h,b){for(var g=h.rects,k=[],T=g[0].x,y=g[0].y,E=g[0].x+g[0].width,u=g[0].y+g[0].height,f=0;f<g.length;f++){var x=g[f],v=x.x,C=x.x+x.width,N=x.y,A=x.y+x.height;k.push(v,A,C,A,v,N,C,N),T=Math.min(T,v),y=Math.min(y,N),E=Math.max(E,C),u=Math.max(u,A)}var U=n.context.obj({Type:c.PDFName.of("Annot"),Subtype:c.PDFName.of("Highlight"),Rect:n.context.obj([T,y,E,u]),QuadPoints:n.context.obj(k),C:n.context.obj(b),F:c.PDFNumber.of(4),T:c.PDFString.of("PDF Annotator"),M:c.PDFString.of(new Date().toISOString()),CA:c.PDFNumber.of(.4)});h.note&&U.set(c.PDFName.of("Contents"),c.PDFString.of(h.note));var I=s(c,n,g,b,[T,y,E,u]);U.set(c.PDFName.of("AP"),n.context.obj({N:I}));var O=n.context.register(U),$=[O];if(h.note){var D=n.context.register(n.context.obj({Type:c.PDFName.of("Annot"),Subtype:c.PDFName.of("Popup"),Rect:n.context.obj([E+8,y-60,E+208,y+12]),Parent:O,Open:!1}));U.set(c.PDFName.of("Popup"),D),$.push(D)}return $}function l(c,n,h){var b=n.node.get(c.PDFName.of("Annots"));if(b instanceof c.PDFArray)for(var g=0;g<h.length;g++)b.push(h[g]);else n.node.set(c.PDFName.of("Annots"),n.doc.context.obj(h))}async function i(c,n,h,b){for(var g=await c.PDFDocument.load(n),k=g.getPages(),T=h||[],y=0;y<T.length;y++){var E=T[y];if(!(!E||!E.rects||!E.rects.length)){var u=k[E.page-1];if(u){var f=b&&b[E.color]||t,x=a(c,g,E,f);l(c,u,x)}}}return g.save()}return{writeHighlightsIntoPdf:i,buildHighlightAnnotation:a,appendAnnotationRefs:l}}var ke=Ae(),Xn=ke.writeHighlightsIntoPdf,Yn=ke.buildHighlightAnnotation,Zn=ke.appendAnnotationRefs;function Ie(){function t(n){return String(n??"").replace(/\]/g,"\\]")}function s(n,h,b,g,k){var T=new URLSearchParams;h&&T.set("att",h),Number.isFinite(b)&&b>=1&&T.set("page",String(Math.floor(b))),g&&T.set("hl",g),k&&T.set("note",k);var y=T.toString();return"plugin://"+n+(y?"?"+y:"")}function a(n,h){return String(n??"").split(/\r?\n/).map(function(b){return(h+" "+b).replace(/[ \t]+$/,"")})}function l(n,h,b,g,k,T){var y=s(h,b,g.page,g.id,T),E=t(n||"PDF"),u='==\u25CF<!-- {"cycleColor":"'+k+'"} -->==',f=u+" ["+E+"]("+y+")",x=[f].concat(a(g.quoteText,"> >"));return g.note&&(x.push(">"),x=x.concat(a(g.note,">"))),x.join(`
`)}function i(n){return n.slice().sort(function(h,b){if(h.page!==b.page)return h.page-b.page;var g=h.rects&&h.rects[0]?h.rects[0].y:0,k=b.rects&&b.rects[0]?b.rects[0].y:0;return k-g})}function c(n,h,b,g,k,T,y){var E=T&&T.length?T:null,u=(g||[]).filter(function(v){return v&&(!E||E.indexOf(v.color)!==-1)}),f=i(u),x=f.map(function(v){var C=k?k[v.color]:void 0;return l(n,h,b,v,C,y)});return x.join(`

`)}return{buildDeepLink:s,buildHighlightBlock:l,buildExportAllContent:c}}var De=Ie(),Kn=De.buildDeepLink,eo=De.buildHighlightBlock,to=De.buildExportAllContent;function ut(){var t=window.__PDFA_CONFIG||{},s=window.__PDFA_GEOM||{},a=window.__PDFA_ANNOTATIONS||{},l=window.__PDFA_EXPORT||{},i={root:document.getElementById("pdfa-root"),pages:document.getElementById("pdfa-pages"),status:document.getElementById("pdfa-status"),pageLabel:document.getElementById("pdfa-page-label"),zoomLabel:document.getElementById("pdfa-zoom-label"),colors:document.getElementById("pdfa-colors"),hint:document.getElementById("pdfa-hint"),popover:document.getElementById("pdfa-popover"),panel:document.getElementById("pdfa-panel"),listToggle:document.getElementById("pdfa-list-toggle"),count:document.getElementById("pdfa-count"),more:document.getElementById("pdfa-more"),open:document.getElementById("pdfa-open"),scrollUp:document.getElementById("pdfa-scroll-up"),scrollDown:document.getElementById("pdfa-scroll-down"),collapsedCount:document.getElementById("pdfa-collapsed-count"),collapsedName:document.querySelector(".pdfa-collapsed-name"),name:document.querySelector(".pdfa-name")};function c(e){e&&(n.attachmentName=e,i.name&&(i.name.textContent=e),i.collapsedName&&(i.collapsedName.textContent=e))}var n={doc:null,scale:1.25,pageCount:0,current:1,rendering:!1,textSpans:0,viewports:{},highlights:[],pdfBytes:null,attachmentName:t.attachmentName||"",activeColorId:t.defaultColorId||((t.colors||[{}])[0]||{}).id,pendingSelection:null,lastCapturedText:"",noteEditing:null};function h(e,o){i.status.textContent=e||"",i.status.style.display=e?"block":"none",i.status.className=o?"pdfa-status pdfa-error":"pdfa-status"}function b(e){var o=Object.assign({noteUUID:t.noteUUID},e);return new Promise(function(r,d){try{if(typeof window.callAmplenotePlugin!="function")throw new Error("Plugin bridge unavailable (callAmplenotePlugin missing)");r(window.callAmplenotePlugin(JSON.stringify(o)))}catch(p){d(p)}}).then(function(r){if(r&&typeof r=="object")return r;if(typeof r!="string")throw new Error("Empty reply from the plugin");try{return JSON.parse(r)}catch{throw new Error("Unreadable reply from the plugin: "+String(r).slice(0,120))}})}function g(){return t.colors||[]}function k(e){for(var o=g(),r=0;r<o.length;r++)if(o[r].id===e)return o[r].hex;return o.length?o[0].hex:"#F4DE6C"}function T(e){for(var o=0;o<n.highlights.length;o++)if(n.highlights[o].id===e)return n.highlights[o];return null}function y(e,o,r){var d=document.createElement("button");return d.className="pdfa-btn"+(o?" "+o:""),d.textContent=e,d.onclick=function(p){p.stopPropagation(),r()},d}function E(e,o,r,d){var p=document.createElement("button");return p.className="pdfa-color",p.dataset.color=e.id,p.style.background=e.hex,p.title=d+" "+e.label,p.setAttribute("aria-label",d+" "+e.label),p.setAttribute("aria-pressed",String(!!o)),p.onclick=function(m){m.stopPropagation(),r(e.id)},p}function u(){for(var e=g(),o=0;o<e.length;o++)i.colors.appendChild(E(e[o],e[o].id===n.activeColorId,function(r){n.activeColorId=r,f(),n.pendingSelection&&Pe(n.pendingSelection,r)},"Highlight"))}function f(){for(var e=i.colors.querySelectorAll(".pdfa-color"),o=0;o<e.length;o++)e[o].setAttribute("aria-pressed",String(e[o].dataset.color===n.activeColorId))}function x(e,o){var r=e.getViewport({scale:n.scale});n.viewports[o]=r;var d=document.createElement("div");d.className="pdfa-page",d.dataset.page=String(o),d.style.width=r.width+"px",d.style.height=r.height+"px";var p=document.createElement("canvas"),m=window.devicePixelRatio||1;p.width=Math.floor(r.width*m),p.height=Math.floor(r.height*m),p.style.width=r.width+"px",p.style.height=r.height+"px",d.appendChild(p);var w=document.createElement("div");w.className="pdfa-highlights",d.appendChild(w);var S=document.createElement("div");S.className="textLayer",S.style.width=r.width+"px",S.style.height=r.height+"px",S.style.setProperty("--scale-factor",String(n.scale)),d.appendChild(S),i.pages.appendChild(d);var H=p.getContext("2d");return H.scale(m,m),e.render({canvasContext:H,viewport:r}).promise.then(function(){return e.getTextContent()}).then(function(F){var P=[];return window.pdfjsLib.renderTextLayer({textContent:F,container:S,viewport:r,textDivs:P}).promise.then(function(){n.textSpans+=P.length;for(var L=0;L<P.length;L++)P[L].__pdfaItem=F.items[L];N(o)})})}function v(){if(n.rendering)return Promise.resolve();n.rendering=!0,M(!0),i.pages.innerHTML="",n.viewports={},n.textSpans=0,h("Rendering...");for(var e=Promise.resolve(),o=1;o<=n.pageCount;o++)(function(r){e=e.then(function(){return n.doc.getPage(r).then(function(d){return x(d,r)})})})(o);return e.then(function(){n.textSpans===0?h("No selectable text found - this PDF may be a scan.",!0):h(""),n.rendering=!1,K(),xe()}).catch(function(r){n.rendering=!1,h("Failed to render: "+r.message,!0)})}function C(e){return function(o,r){return e.convertToViewportPoint(o,r)}}function N(e){for(var o=e?'.pdfa-page[data-page="'+e+'"]':".pdfa-page",r=i.pages.querySelectorAll(o),d=0;d<r.length;d++){var p=r[d],m=Number(p.dataset.page),w=p.querySelector(".pdfa-highlights"),S=n.viewports[m];if(!(!w||!S)){w.innerHTML="";for(var H=C(S),F=0;F<n.highlights.length;F++){var P=n.highlights[F];if(!(!P||P.page!==m||!P.rects||!P.rects.length)){var L=document.createElement("div");L.className="pdfa-hl-group",L.dataset.id=P.id||"";for(var V=0;V<P.rects.length;V++){var Y=s.pdfRectToViewportRect(P.rects[V],H),_=document.createElement("div");_.className="pdfa-hl",_.style.left=Y.x+"px",_.style.top=Y.y+"px",_.style.width=Y.width+"px",_.style.height=Y.height+"px",_.style.background=k(P.color),L.appendChild(_)}w.appendChild(L)}}}}}function A(){N(),I(),i.count.textContent=String(n.highlights.length)}function U(){return n.highlights.slice().sort(function(e,o){return e.page!==o.page?e.page-o.page:(o.rects[0]?o.rects[0].y:0)-(e.rects[0]?e.rects[0].y:0)})}function I(){i.panel.innerHTML="";var e=document.createElement("div");e.className="pdfa-panel-title";var o=document.createElement("span");o.textContent="Highlights",e.appendChild(o),e.appendChild(y("Close","",function(){$(!1)})),i.panel.appendChild(e);var r=U();if(!r.length){var d=document.createElement("div");d.className="pdfa-panel-empty",d.textContent="No highlights yet. Select some text in the PDF and pick a color.",i.panel.appendChild(d);return}for(var p=0;p<r.length;p++)i.panel.appendChild(O(r[p]))}function O(e){var o=document.createElement("div");o.className="pdfa-hl-row",o.dataset.id=e.id||"",o.title="Jump to this highlight";var r=document.createElement("span");r.className="pdfa-chip",r.style.background=k(e.color),o.appendChild(r);var d=document.createElement("div"),p=document.createElement("div");p.className="pdfa-hl-page",p.textContent="Page "+e.page,d.appendChild(p);var m=document.createElement("div");if(m.className="pdfa-hl-quote",m.textContent=e.quoteText.length>160?e.quoteText.slice(0,160)+"...":e.quoteText,d.appendChild(m),e.note){var w=document.createElement("div");w.className="pdfa-hl-note",w.textContent=e.note,d.appendChild(w)}return o.appendChild(d),o.onclick=function(){Re(e)},o}function $(e){var o=e===void 0?!i.panel.classList.contains("pdfa-open"):e;i.panel.classList.toggle("pdfa-open",o),i.listToggle.setAttribute("aria-pressed",String(o)),o&&I()}function D(e){for(var o=e&&e.nodeType===1?e:e&&e.parentElement;o;){if(o.classList&&o.classList.contains("textLayer"))return o;o=o.parentElement}return null}function Q(e,o){for(var r=[],d=[],p=null,m=document.createTreeWalker(o,NodeFilter.SHOW_TEXT,null),w;w=m.nextNode();)if(e.intersectsNode(w)){var S=w.nodeValue||"",H=w===e.startContainer?e.startOffset:0,F=w===e.endContainer?e.endOffset:S.length,P=w.parentElement,L=P&&P.__pdfaItem;if(L)for(var V={x1:L.transform[4],y1:L.transform[5],x2:L.transform[4]+L.width,y2:L.transform[5]+L.height},Y=P.getBoundingClientRect(),_=s.textTokenRanges(S,H,F),Z=0;Z<_.length;Z++){var be=document.createRange();be.setStart(w,_[Z].start),be.setEnd(w,_[Z].end);var q=s.unionClientRects(be.getClientRects());if(q){var ze={left:q.left,top:q.top,width:q.width,height:q.height,right:q.left+q.width,bottom:q.top+q.height},je=s.itemRelativeRect(V,Y,ze);je&&(r.push(je),d.push(S.slice(_[Z].start,_[Z].end)),p=ze)}}}return{rects:r,text:d.join(" "),lastCssRect:p}}function R(e){if(n.pendingSelection=e,n.lastCapturedText=e&&e.rawText||"",!e){i.hint.textContent="",i.hint.style.display="none";return}i.hint.textContent=e.spilled?"Pick a color (page "+e.page+" only)":"Pick a color",i.hint.style.display="inline"}function oe(e){if(!n.noteEditing){var o=window.getSelection();if(!o||o.isCollapsed||o.rangeCount===0){R(null),M();return}var r=o.getRangeAt(0),d=D(r.startContainer);if(!d)return R(null);var p=d.parentElement;if(!p||!p.dataset||!p.dataset.page)return R(null);var m=Number(p.dataset.page);if(!n.viewports[m])return R(null);var w=D(r.endContainer)!==d,S=Q(r,d),H=s.mergeLineRects(S.rects);if(!H.length)return R(null);var F=S.lastCssRect||p.getBoundingClientRect(),P=e&&e.clientX?e.clientX:F.left+F.width/2,L=e&&e.clientY?e.clientY:F.top+F.height,V={page:m,rects:H,quoteText:s.normalizeQuoteText(S.text),spilled:w,anchorX:P,anchorY:L,rawText:String(o)};R(V),wt(V)}}var fe=300,B=null;function G(){n.noteEditing||(B&&clearTimeout(B),B=setTimeout(W,fe))}function W(){if(B=null,!n.noteEditing){var e=window.getSelection();!e||e.isCollapsed||e.rangeCount===0||D(e.getRangeAt(0).startContainer)&&String(e)!==n.lastCapturedText&&oe(null)}}function ae(e,o){var r=n.highlights;return n.highlights=e,A(),b(o).then(function(d){if(!d||d.error)throw new Error(d&&d.error||"The plugin did not confirm the change.");return n.highlights=d.highlights||e,A(),h(""),!0}).catch(function(d){return n.highlights=r,A(),h(d.message||String(d),!0),!1})}function Pe(e,o){var r={id:null,page:e.page,color:o,rects:e.rects,quoteText:e.quoteText,note:null},d=e.anchorX,p=e.anchorY;R(null),M(!0);var m=window.getSelection();m&&m.removeAllRanges&&m.removeAllRanges(),ae(n.highlights.concat([r]),{action:"addHighlight",attachmentUUID:t.attachmentUUID,highlight:r}).then(function(w){if(w){var S=n.highlights[n.highlights.length-1];S&&S.id&&me(S,d,p,!0)}})}function mt(e,o){M(!0),ae(n.highlights.map(function(r){return r.id===e?Object.assign({},r,{color:o}):r}),{action:"recolorHighlight",attachmentUUID:t.attachmentUUID,id:e,color:o})}function vt(e){M(!0),ae(n.highlights.filter(function(o){return o.id!==e}),{action:"removeHighlight",attachmentUUID:t.attachmentUUID,id:e})}function ge(e,o){var r=String(o??"").trim();n.noteEditing=null,M(!0),ae(n.highlights.map(function(d){return d.id===e?Object.assign({},d,{note:r||null}):d}),{action:"setHighlightNote",attachmentUUID:t.attachmentUUID,id:e,note:r})}function X(e,o,r,d){i.popover.innerHTML="",i.popover.classList.toggle("pdfa-editing",d==="editing"),i.popover.classList.toggle("pdfa-exporting",d==="exporting"),i.popover.classList.toggle("pdfa-menu",d==="menu");for(var p=0;p<e.length;p++)i.popover.appendChild(e[p]);i.popover.classList.add("pdfa-open");var m=i.popover.offsetWidth,w=i.popover.offsetHeight,S=Math.max(4,Math.min(o-m/2,window.innerWidth-m-4)),H=r+12;H+w>window.innerHeight-4&&(H=Math.max(4,r-w-12)),i.popover.style.left=S+"px",i.popover.style.top=H+"px"}function M(e){n.noteEditing&&!e||(n.noteEditing=null,i.popover.classList.remove("pdfa-open","pdfa-editing","pdfa-exporting","pdfa-menu"),i.popover.innerHTML="")}function wt(e){for(var o=g(),r=[],d=0;d<o.length;d++)r.push(E(o[d],o[d].id===n.activeColorId,function(p){n.activeColorId=p,f(),Pe(e,p)},"Highlight"));X(r,e.anchorX,e.anchorY)}function me(e,o,r,d){for(var p=g(),m=[],w=0;w<p.length;w++)m.push(E(p[w],p[w].id===e.color,function(H){mt(e.id,H)},"Change to"));var S=!!e.note;m.push(y(S?"Edit note":"Add note",d&&!S?"pdfa-btn-primary":"",function(){bt(e,o,r)})),m.push(y("Copy","",function(){It(e)})),m.push(y("Send to note","",function(){Dt(e)})),m.push(y("Remove","pdfa-remove",function(){vt(e.id)})),X(m,o,r)}function xt(e,o){for(var r=g(),d={},p=0;p<r.length;p++)d[r[p].id]=!0;var m=document.createElement("div");m.className="pdfa-export-hint",m.textContent="Export highlights to a note";var w=document.createElement("div");w.className="pdfa-export-colors";for(var S=0;S<r.length;S++)(function(F){var P=E(F,!0,function(L){d[L]=!d[L],P.setAttribute("aria-pressed",String(d[L]))},"Toggle");w.appendChild(P)})(r[S]);var H=document.createElement("div");H.className="pdfa-note-actions",H.appendChild(y("Create / update note","pdfa-btn-primary",function(){for(var F=[],P=0;P<r.length;P++)d[r[P].id]&&F.push(r[P].id);Pt(F.length===r.length?null:F)})),X([m,w,H],e,o,"exporting")}function bt(e,o,r){n.noteEditing=e.id;var d=document.createElement("textarea");d.className="pdfa-note-input",d.rows=3,d.value=e.note||"",d.placeholder="Note for this highlight";var p=document.createElement("div");p.className="pdfa-note-actions",e.note&&p.appendChild(y("Delete note","",function(){ge(e.id,"")}));var m=document.createElement("span");m.className="pdfa-spacer",p.appendChild(m),p.appendChild(y("Cancel","",function(){Ue(e,o,r)})),p.appendChild(y("Save","pdfa-btn-primary",function(){ge(e.id,d.value)})),d.onkeydown=function(w){w.key==="Enter"&&(w.ctrlKey||w.metaKey)?(w.preventDefault(),w.stopPropagation(),ge(e.id,d.value)):w.key==="Escape"&&(w.preventDefault(),w.stopPropagation(),Ue(e,o,r))},X([d,p],o,r,"editing"),d.focus(),d.setSelectionRange(d.value.length,d.value.length)}function Ue(e,o,r){n.noteEditing=null;var d=T(e.id)||e;me(d,o,r)}function yt(e){if(!n.noteEditing){var o=window.getSelection();if(!(o&&!o.isCollapsed)){for(var r=e.target,d=null;r&&r!==i.pages;){if(r.classList&&r.classList.contains("pdfa-page")){d=r;break}r=r.parentElement}if(!d)return M();var p=Number(d.dataset.page),m=n.viewports[p];if(!m)return M();var w=d.getBoundingClientRect(),S=m.convertToPdfPoint(e.clientX-w.left,e.clientY-w.top),H=s.hitTestHighlights(n.highlights,p,S[0],S[1],1);H&&H.id?me(H,e.clientX,e.clientY):M()}}}function K(){i.pageLabel.textContent=n.current+" / "+n.pageCount,i.zoomLabel.textContent=Math.round(n.scale*100)+"%"}function ee(){return i.root.querySelector(".pdfa-scroll")}function ve(e){var o=Math.min(Math.max(1,e),n.pageCount),r=i.pages.querySelector('[data-page="'+o+'"]');r&&r.scrollIntoView({behavior:"smooth",block:"start"}),n.current=o,K()}function Re(e){var o=i.pages.querySelector('.pdfa-page[data-page="'+e.page+'"]'),r=n.viewports[e.page];if(!(!o||!r||!e.rects||!e.rects.length)){var d=s.pdfRectToViewportRect(e.rects[0],C(r)),p=ee(),m=o.getBoundingClientRect().top+d.y;p.scrollTop+=m-p.getBoundingClientRect().top-p.clientHeight/3,n.current=e.page,K()}}function we(e){return n.scale=Math.min(Math.max(.4,e),4),v()}function Ct(){return n.doc?n.doc.getPage(1).then(function(e){var o=ee();if(o){var r=window.getComputedStyle(o),d=o.clientWidth-(parseFloat(r.paddingLeft)||0)-(parseFloat(r.paddingRight)||0),p=e.getViewport({scale:1}).width;if(!(!(d>0)||!(p>0))){var m=Math.max(.4,d/p);m<n.scale&&(n.scale=m,K())}}}).catch(function(){}):Promise.resolve()}function He(e){var o=ee();o&&(o.scrollTop+=e*Math.max(80,o.clientHeight*.85),xe())}function xe(){var e=ee();if(!(!e||!i.scrollUp)){var o=e.scrollHeight-e.clientHeight;i.scrollUp.disabled=e.scrollTop<=1,i.scrollDown.disabled=e.scrollTop>=o-1}}function Et(){xe(),M();for(var e=i.pages.querySelectorAll(".pdfa-page"),o=n.current,r=1/0,d=0;d<e.length;d++){var p=Math.abs(e[d].getBoundingClientRect().top-60);p<r&&(r=p,o=Number(e[d].dataset.page))}o!==n.current&&(n.current=o,K())}function Nt(){return new Promise(function(e,o){if(window.pdfjsLib)return e(window.pdfjsLib);var r=document.createElement("script");r.src=t.pdfJsSrc,r.onload=function(){window.pdfjsLib?e(window.pdfjsLib):o(new Error("PDF.js loaded but did not register itself."))},r.onerror=function(){o(new Error("Could not load PDF.js from the CDN."))},document.head.appendChild(r)})}function Tt(){return new Promise(function(e,o){if(window.PDFLib)return e(window.PDFLib);var r=document.createElement("script");r.src=t.pdfLibSrc,r.onload=function(){window.PDFLib?e(window.PDFLib):o(new Error("pdf-lib loaded but did not register itself."))},r.onerror=function(){o(new Error("Could not load pdf-lib from the CDN."))},document.head.appendChild(r)})}function St(){for(var e={},o=g(),r=0;r<o.length;r++)o[r].rgb&&(e[o[r].id]=o[r].rgb);return e}function At(){var e=(n.attachmentName||"annotated").replace(/\.pdf$/i,"");return e+"-annotated.pdf"}function Le(){for(var e={},o=g(),r=0;r<o.length;r++)o[r].cycleIndex!==void 0&&(e[o[r].id]=o[r].cycleIndex);return e}function Me(){var e=(n.attachmentName||"PDF").replace(/\.pdf$/i,"");return e+" - Highlights"}function Fe(e){return l.buildHighlightBlock(n.attachmentName,t.pluginUUID,t.attachmentUUID,e,Le()[e.color],t.noteUUID)}function kt(e){return navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(e):new Promise(function(o,r){var d=document.createElement("textarea");d.value=e,d.style.position="fixed",d.style.left="-9999px",document.body.appendChild(d),d.focus(),d.select();var p=!1;try{p=document.execCommand("copy")}catch{p=!1}document.body.removeChild(d),p?o():r(new Error("Clipboard access is unavailable here."))})}function It(e){M(!0),kt(Fe(e)).then(function(){h("Highlight copied - paste it into any note.")}).catch(function(o){h("Could not copy: "+(o.message||o),!0)})}function Dt(e){M(!0),b({action:"sendToNote",content:Fe(e)}).then(function(o){if(!o||o.error)throw new Error(o&&o.error||"Could not send this to the note.");h("Sent to the bottom of this note.")}).catch(function(o){h(o.message||String(o),!0)})}function Pt(e){M(!0);var o=l.buildExportAllContent(n.attachmentName,t.pluginUUID,t.attachmentUUID,n.highlights,Le(),e,t.noteUUID);if(!o){h(e?"No highlights match those colors.":"No highlights to export yet.",!0);return}b({action:"exportAll",noteName:Me(),content:o}).then(function(r){if(!r||r.error)throw new Error(r&&r.error||"Could not export highlights.");h('Exported to "'+Me()+'".')}).catch(function(r){h(r.message||String(r),!0)})}function Oe(e,o){var r=document.createElement("div");r.className="pdfa-menu-name",r.textContent=n.attachmentName||"PDF Annotator",r.title=r.textContent;var d=[r];window.matchMedia&&window.matchMedia("(max-width: 520px)").matches&&d.push(Ut(e,o)),d.push(y("Collapse","",function(){M(!0),Ft()}),y("Download","",function(){M(!0),Lt()}),y("Export...","",function(){xt(e,o)}),y("Remove viewer...","pdfa-remove",function(){Rt(e,o)})),X(d,e,o,"menu")}function Ut(e,o){var r=document.createElement("div");r.className="pdfa-menu-zoom";var d=document.createElement("span");d.className="pdfa-menu-zoom-label",d.textContent=Math.round(n.scale*100)+"%";var p=function(S){return function(){we(n.scale+S).then(function(){Oe(e,o)})}},m=y("\u2212","",p(-.25)),w=y("+","",p(.25));return m.title="Zoom out",w.title="Zoom in",m.disabled=n.scale<=.4,w.disabled=n.scale>=4,r.appendChild(m),r.appendChild(d),r.appendChild(w),r}function Rt(e,o){var r=document.createElement("div");r.className="pdfa-export-hint",r.textContent="Remove this viewer and all its highlights from this note? This cannot be undone.";var d=document.createElement("div");d.className="pdfa-note-actions",d.appendChild(y("Cancel","",function(){M(!0)}));var p=document.createElement("span");p.className="pdfa-spacer",d.appendChild(p),d.appendChild(y("Remove","pdfa-remove",Ht)),X([r,d],e,o,"exporting")}function Ht(){M(!0),h("Removing this viewer..."),b({action:"removeViewer",attachmentUUID:t.attachmentUUID,pluginUUID:t.pluginUUID}).then(function(e){if(!e||e.error)throw new Error(e&&e.error||"Could not remove this viewer.");document.body.innerHTML='<div style="padding:16px;font:13px sans-serif;opacity:.75">Removed - this block will disappear once the note refreshes.</div>'}).catch(function(e){h(e.message||String(e),!0)})}function Lt(){n.pdfBytes&&(h("Preparing the download..."),Tt().then(function(e){return a.writeHighlightsIntoPdf(e,n.pdfBytes,n.highlights,St())}).then(function(e){var o=new Blob([e],{type:"application/pdf"}),r=URL.createObjectURL(o),d=document.createElement("a");d.href=r,d.download=At(),document.body.appendChild(d),d.click(),d.remove(),setTimeout(function(){URL.revokeObjectURL(r)},4e3),h("")}).catch(function(e){h("Could not prepare the download: "+(e.message||e),!0)}))}function Mt(){return b({action:"loadHighlights",attachmentUUID:t.attachmentUUID}).then(function(e){if(!e||e.error)throw new Error(e&&e.error||"No answer from the plugin");n.highlights=e.highlights||[]}).catch(function(e){n.highlights=[],h("Could not load saved highlights: "+(e.message||e),!0)})}function Ft(){var e=n.highlights.length;i.collapsedCount.textContent=e?e+(e===1?" highlight":" highlights"):"",i.root.classList.add("pdfa-collapsed-mode"),$e(!0)}function $e(e){b({action:"setCollapsed",collapsed:e,attachmentUUID:t.attachmentUUID,pluginUUID:t.pluginUUID}).catch(function(){})}function Ot(){i.root.classList.remove("pdfa-collapsed-mode"),n.doc||_e(),$e(!1)}function _e(){h("Loading PDF..."),Nt().then(function(e){return e.GlobalWorkerOptions.workerSrc=t.workerSrc,b({action:"getPdfUrl",attachmentUUID:t.attachmentUUID})}).then(function(e){if(!e||!e.url)throw new Error(e&&e.error||"Could not resolve the PDF URL");return c(e.name),fetch(e.url)}).then(function(e){if(!e.ok)throw new Error("Download failed (HTTP "+e.status+")");return e.arrayBuffer()}).then(function(e){return n.pdfBytes=e.slice(0),window.pdfjsLib.getDocument({data:e}).promise}).then(function(e){return n.doc=e,n.pageCount=e.numPages,Mt()}).then(function(){return Ct()}).then(function(){return v()}).then(function(){A();var e=t.highlightId?T(t.highlightId):null;e?Re(e):t.page&&ve(t.page)}).catch(function(e){h(e.message||String(e),!0)})}try{document.getElementById("pdfa-prev").onclick=function(){ve(n.current-1)},document.getElementById("pdfa-next").onclick=function(){ve(n.current+1)},document.getElementById("pdfa-zoom-in").onclick=function(){we(n.scale+.25)},document.getElementById("pdfa-zoom-out").onclick=function(){we(n.scale-.25)},i.scrollUp.onclick=function(){He(-1)},i.scrollDown.onclick=function(){He(1)},i.listToggle.onclick=function(){$()},i.more.onclick=function(e){Oe(e.clientX,e.clientY)},ee().addEventListener("scroll",Et),i.pages.addEventListener("mouseup",oe),i.pages.addEventListener("click",yt),document.addEventListener("selectionchange",G),i.pages.addEventListener("touchend",function(){B&&clearTimeout(B),B=null,W()}),document.addEventListener("keydown",function(e){e.key==="Escape"&&!n.noteEditing&&M()}),document.addEventListener("mousedown",function(e){i.popover.classList.contains("pdfa-open")&&(i.popover.contains(e.target)||M())}),u(),I(),i.root.querySelector(".pdfa-collapsed").onclick=Ot,t.collapsed?b({action:"getViewerSummary",attachmentUUID:t.attachmentUUID}).then(function(e){if(!(!e||e.error)){c(e.name);var o=e.count||0;i.collapsedCount.textContent=o?o+(o===1?" highlight":" highlights"):""}}).catch(function(){}):_e()}catch(e){h("Viewer failed to start: "+(e&&e.message?e.message:e),!0)}}function tn(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function nn(t){return JSON.stringify(t).replace(/</g,"\\u003c")}var on=`
...
<div id="pdfa-root"${h?' class="pdfa-collapsed-mode"':""}>
  <div class="pdfa-collapsed">
    ...
    <span class="pdfa-collapsed-name">${tn(s)}</span>
...
<script>window.__PDFA_CONFIG = ${nn(g)};
window.__PDFA_GEOM = (${Se.toString()})();
window.__PDFA_ANNOTATIONS = (${Ae.toString()})();
window.__PDFA_EXPORT = (${Ie.toString()})();<\/script>
<script>(${ut.toString()})();<\/script>`}var an={noteOption:{"Annotate PDF":async function(t,s){return Ke(t,s,t.context.pluginUUID)},"Debug: dump note markdown":async function(t,s){return et(t,s)}},insertText:async function(t){return tt(t,t.context.noteUUID,t.context.pluginUUID)},linkTarget:async function(t,...s){return nt(t,s[0])},renderEmbed:function(t,...s){let{attachmentUUID:a,page:l,highlightId:i,collapsed:c,attachmentName:n}=ne(s[0]);return a?gt({attachmentUUID:a,page:l,highlightId:i,collapsed:c,attachmentName:n,lightDarkMode:t.context.lightDarkMode,pluginUUID:t.context.pluginUUID,noteUUID:t.context.noteUUID}):`<p style="font:13px sans-serif;padding:12px">
        This viewer isn't linked to a PDF. Re-run <b>Annotate PDF</b> from the note menu.
      </p>`},onEmbedCall:async function(t,...s){return pt(t,s[0])}},rn=an;return qt(sn);})();
