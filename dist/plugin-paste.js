...
\`\`\`${mn}
${l}
\`\`\``}function Ae(t){if(!t)return null;let l=t.match(/```(?:json)?\s*\n?([\s\S]*?)\n?```/),r=!l&&t.match(/<!--\s*PDFA-DATA\s*\n?([\s\S]*?)-->/),s=(l?l[1]:r?r[1]:t).trim();if(!s)return null;try{return JSON.parse(s)}catch{return null}}function vn(t){if(!Array.isArray(t))return[];let l=[];for(let r of t)try{l.push(ge(r))}catch{}return l}async function me(t,l,r){let s=await t.getNoteContent({uuid:l}),i=ke(s,G),d=Ae(i);return!d||typeof d!="object"?[]:vn(d[r])}async function gt(t,l,r,s){let i={uuid:l},d=await t.getNoteContent(i),o=ke(d,G),A={...Ae(o)||{},[r]:s},S=ft(A);o===null&&await t.insertNoteContent(i,`

# ${G}

`,{atEnd:!0});let y=xn(d,S);if(y!==null){await t.replaceNoteContent(i,y);return}await t.replaceNoteContent(i,S,{section:{heading:{text:G,level:1}}})}async function mt(t,l,r){let s={uuid:l},i=await t.getNoteContent(s),d=ke(i,G);if(d===null)return;let o=Ae(d)||{};if(!(r in o))return;let p={...o};delete p[r],await t.replaceNoteContent(s,ft(p),{section:{heading:{text:G,level:1}}})}function Ie(t,l){let r=/^#\s+(.*)$/,s=t.findIndex(d=>{let o=d.match(r);return o&&o[1].trim()===l});if(s===-1)return null;let i=t.length;for(let d=s+1;d<t.length;d++)if(/^#\s+/.test(t[d])){i=d;break}return{start:s,end:i}}function ke(t,l){if(!t)return null;let r=t.split(`
`),s=Ie(r,l);return s?r.slice(s.start+1,s.end).join(`
...
`),s=Ie(r,G);if(!s)return null;let i=r.slice(0,s.start).join(`
`).replace(/\s+$/,""),d=r.slice(s.start).join(`
`);return`${i?i+`

`:""}${l}

${d}`}function xn(t,l){let r=(t||"").split(`
`),s=Ie(r,G);if(!s)return null;let i=wn(r.slice(s.start+1,s.end).join(`
